"""
core/experiments.py — 参数化实验批量运行 + 可追溯帕累托分析  v4.2.2
================================================================================
...
def build_pareto3d_figure(
    ...
    采用原始指标值构建透视3D图，返回(fig, ax)：
      X=连续采光达标度Cd（旧CSV回退Ra）、Y=所选热指标、Z=成本。
    ...
    xmin, xmax = _axis_limits(d[light_col])
    ...
    for grp, key, mk, sub in _color_subgroups(comp_df):
        ...
        if len(non_pf):
            ax.scatter(non_pf[light_col], non_pf[thermal_col], non_pf["cost"],
                       s=marker_area, marker=mk, c=col, alpha=0.34,
                       edgecolors="none", depthshade=False)
        if len(pf_pts):
            ax.scatter(pf_pts[light_col], pf_pts[thermal_col], pf_pts["cost"],
                       s=max(1.2, marker_area), marker=mk, c=col, alpha=0.72,
                       edgecolors="none", depthshade=False)
    ncomp = d[~compliant]
    if len(ncomp):
        ax.scatter(ncomp[light_col], ncomp[thermal_col], ncomp["cost"],
    ...
    if show_projections:
        for grp, key, mk, sub in _color_subgroups(comp_df):
            col = sub["_plot_color"].iloc[0] if has_color else _C_ACCENT
            ax.scatter(sub[light_col], sub[thermal_col], np.full(len(sub), z_plane),
                       s=max(0.6, marker_area * 0.65), c=col, marker=mk,
                       alpha=0.18, edgecolors="none", depthshade=False)
            ax.scatter(sub[light_col], np.full(len(sub), y_plane), sub["cost"],
                       s=max(0.6, marker_area * 0.65), c=col, marker=mk,
                       alpha=0.18, edgecolors="none", depthshade=False)
    ...
    if show_pareto and show_projections and not shading.empty:
        ...
        fronts = [
            (pareto_front(shading, light_col, thermal_col, True, maximize_thermal,
                          u0_col, u0_min), light_col, thermal_col, "xy"),
            (pareto_front(shading, light_col, "cost", True, False,
                          u0_col, u0_min), light_col, "cost", "xz"),
    ...
    if show_pareto and not pf_global.empty:
        ...
        ring_kw = dict(s=max(ring_area, marker_area * 4.0), marker="o", facecolors="none",
                       ...
                       depthshade=False)
        ax.scatter(p[light_col], p[thermal_col], p["cost"], **ring_kw)
    ...
    if not recommended.empty:
        ...
        ax.scatter([r[light_col]], [r[thermal_col]], [r["cost"]], **star_kw)
        if show_projections:
            proj_star = dict(star_kw); proj_star.update(s=58, alpha=0.68)
            ax.scatter([r[light_col]], [r[thermal_col]], [z_plane], **proj_star)
            ax.scatter([r[light_col]], [y_plane], [r["cost"]], **proj_star)
    ...
    ax.set_yticks(_axis_ticks(d[thermal_col]))
    ax.set_zticks(_axis_ticks(d["cost"]))
    ...
    ax.set_title(
        ...
        "原始指标透视图｜低热不舒适度、低成本向外；方案编号见下方结果表",
