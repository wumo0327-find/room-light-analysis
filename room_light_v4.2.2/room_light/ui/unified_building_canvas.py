"""Building view rendered directly from the current BP draft source."""
...
from bp_editor.model import DraftDocument, Point, RoomFace, Wall, Window
...
class UnifiedBuildingCanvas(QWidget):
    ...
    def set_model(
        ...
        bp_document: Optional[DraftDocument] = None,
    ) -> None:
        ...
        # v4.1 building view reads the current BP draft directly.  Falling
        # back to the embedded project copy is only for older callers/files.
        document = (
            DraftDocument.from_dict(bp_document.to_dict())
            if bp_document is not None
            else document_from_building(building)[0]
        )
