"""
core/plan_export.py — 房间平面图绘制与测点叠加导出  v2.10.0（v2.8.0 新增，v2.9.1 天正风格标注+遮阳可视化，v2.10.0 支持逐窗/逐位置遮阳覆盖+两端外墙翼板）
======================================================================
...
def _fin_slots(room: RoomModel, wall: str) -> List[Tuple[float, float, float]]:
    ...
    与 core/daylight.py 的 _vertical_fin_positions_mm 使用同一套"每扇窗左右两侧"
    几何口径，只是这里额外算出每处的完整区间宽度用于画外凸矩形。
    """
    sh = room.shading
    ...
    if d_last > 0.0 and wins[-1].right < axis_len - 1e-6:
        slots.append((wins[-1].right, axis_len, d_last))
    return slots
...
def draw_shading_elevation(ax, room: RoomModel, wall: str, fontsize=8):
    ...
    if has_fin:
        ...
        d0 = sh.get_fin_depth_for(wins[0].id, "L")
        if d0 > 0.0 and wins[0].x > 1e-6:
            _draw_fin_rect(0.0, wins[0].x, wins[0].sill, wins[0].head)
        for i in range(len(wins) - 1):
            d_r = sh.get_fin_depth_for(wins[i].id, "R")
            d_l = sh.get_fin_depth_for(wins[i + 1].id, "L")
            if max(d_r, d_l) > 0.0:
        ...
        d_last = sh.get_fin_depth_for(wins[-1].id, "R")
        if d_last > 0.0 and wins[-1].right < axis_len - 1e-6:
            _draw_fin_rect(wins[-1].right, axis_len, wins[-1].sill, wins[-1].head)
