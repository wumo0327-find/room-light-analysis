"""
core/daylight.py — Daylight Factor & Illuminance Engine  v2.10.0
==============================================================
...
def _vertical_fin_positions_mm(win, room: RoomModel):
    """
    v2.7: 返回该窗户两侧垂直翼板/装饰柱近侧面的 (沿墙局部坐标 u(mm), 出挑深度mm)
    列表。v2.10 起：每扇窗左右两侧一律各生成一处翼板（此前仅窗与窗之间的窗间墙
    位置才生成，两端外墙默认不设——该简化假设已按实测反馈取消）。最左窗的左侧、
    最右窗的右侧对应两端外墙位置，其余对应相邻窗户共享的窗间墙位置；翼板近侧面
    正好在该窗自身的边界处，不依赖窗间墙宽度本身。
    出挑深度支持逐位置覆盖（ShadingDevice.get_fin_depth_for，键="{win.id}:L/R"），
    缺失覆盖时使用全局 vertical_fin_depth_mm；深度≤0 的位置不生成遮挡。
    """
    sh = room.shading
    if not getattr(sh, "vertical_fin_enabled", False):
        return []
    positions = []
    d_left  = sh.get_fin_depth_for(win.id, "L")
    d_right = sh.get_fin_depth_for(win.id, "R")
    if d_left > 0.0:
        positions.append((win.x, d_left))        # 左侧翼板近侧面（含左端墙位置）
    if d_right > 0.0:
        positions.append((win.right, d_right))    # 右侧翼板近侧面（含右端墙位置）
    return positions
...
def _ds_point(P: np.ndarray, win, room: RoomModel, ndiv: int = WIN_DIV) -> float:
    ...
    sh = room.shading
    over_depth_mm, over_gap_mm = sh.get_overhang_for(win.id)   # v2.10: 逐窗覆盖
    if getattr(sh, "type", "none") == "horizontal_overhang" and over_depth_mm > 0.0:
        D_m    = over_depth_mm  / 1e3      # 出挑深度 m（水平向外）
        gap_m  = over_gap_mm    / 1e3      # 挑檐下沿高出窗顶的距离 m（贴窗顶=0）
    ...
    #   本版本未实现，仅作定性趋势验证。
    fin_positions = _vertical_fin_positions_mm(win, room)
    if fin_positions:
        ...
        for u_fin_mm, d_fin_mm in fin_positions:
            u_fin = u_fin_mm / 1e3
            Dfin_m = d_fin_mm / 1e3     # v2.10: 该位置的出挑深度（可逐位置覆盖）
            with np.errstate(divide="ignore", invalid="ignore"):
