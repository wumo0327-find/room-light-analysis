"""
validate_daylight.py — 采光实测数据交叉验证命令行入口  v2.7.0
================================================================
...
def _plot_comparison_combined(table: pd.DataFrame, out_path: str, title: str,
    ...
    for line in table["测线"].unique():
        sub = table[table["测线"] == line].sort_values("离窗距离_m")
        color = _LINE_COLORS.get(line, "#2563eb")
        marker = _LINE_MARKERS.get(line, "o")
        ax.plot(sub["离窗距离_m"], sub["DF_程序(%)"], "-", color=color,
                lw=2.0, marker=marker, ms=6, label=f"{line} — 程序计算", zorder=3)
        if has_measured and sub["DF_实测(%)"].notna().any():
            ax.scatter(sub["离窗距离_m"], sub["DF_实测(%)"], color=color,
                       s=70, marker=marker, edgecolors="white", linewidths=1.0,
                       label=f"{line} — 实测", zorder=4)
...
def main():
    ...
    if args.plot_mode == "combined":
        ...
        _plot_comparison_combined(table, png_path, title, has_measured)
    else:
