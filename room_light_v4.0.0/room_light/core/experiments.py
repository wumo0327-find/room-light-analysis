"""
core/experiments.py — 参数化实验批量运行 + 可追溯帕累托分析  v4.0.0
================================================================================
    ...
    **计算优化（重要）**：采光(Ra/U0/DF)只由几何(θ,L,h)决定、与遮阳材料无关
    ...
    for m in (materials or []):
        ...
        if spec is not None:
            ...
            mat_specs.append((canonical_name, spec["k_diff"], spec["color"], unit_price))
    if not mat_specs:
        mat_specs = [("默认(k_diff=0.30)", 0.30, _C_ACCENT, 230.0)]
...
def annotate_global_selection(
    ...
    if {"Ra", thermal_col, "cost"}.issubset(out.columns):
        out["pareto_light_thermal_2d"] = out.index.isin(pareto_front(
            candidates, "Ra", thermal_col, True, maximize_thermal,
            u0_col=u0_col, u0_min=u0_min,
        ).index)
        out["pareto_light_cost_2d"] = out.index.isin(pareto_front(
            candidates, "Ra", "cost", True, False,
            u0_col=u0_col, u0_min=u0_min,
        ).index)
    ...
    if not candidates.empty and {"Ra", thermal_col, "cost"}.issubset(candidates.columns):
        distances = np.sqrt((
            (1.0 - _benefit(candidates["Ra"], True)) ** 2
...
def global_selection_summary(
    ...
    rule = (
        ...
        f"数值等价容差为Ra±0.0001、{thermal_label}±{displayed_thermal_tolerance:g}、"
        "造价±0.5元。"
    )
    ...
    if "material" in candidates.columns:
        ...
        def dominates(other: pd.Series, row: pd.Series) -> bool:
            ...
            strict = (
                float(other["Ra"]) > float(row["Ra"]) + ra_tol
                or thermal_strict
