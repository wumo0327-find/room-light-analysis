...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("建筑室内采光分析工具  v4.0.0  |  统一几何核验版")
    ...
    def _update_title(self):
        ...
        suf = f"  —  {fn}" if fn else ""
        self.setWindowTitle(f"建筑室内采光分析工具  v4.0.0{suf}")
    ...
    def _geometry_ready_for_calculation(self, action_name: str) -> bool:
        ...
        if missing_net:
            QMessageBox.critical(
                ...
                + "、".join(missing_net)
                + "\n\n请用建筑视图中的 BP 编辑器打开原始草图并点击“应用到RL分析模型”。"
                "v4 会据墙宽生成实体墙内表面净空间后再允许计算。",
            )
            return False
    ...
    def _run_experiment(self, params: dict):
        ...
        total = batch_total + len(refine_levels) * len(space_snaps) * (
        ...
        def _run_exp(progress_cb):
            ...
            from core.complex_experiments import (
                _daylight_metrics, build_solution_space,
            ...
            refined: set = set()
            performance_columns = (
                "Ra", "U0", "DF_avg", "E_avg", "comfort_months",
            ...
            for _round in range(4):
                ...
                for index in pending:
                    ...
                    for refine_ndiv in refine_levels:
                        ...
                        for column in ("Ra", "U0", "DF_avg", "E_avg"):
