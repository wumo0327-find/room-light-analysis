...
class V4ParetoTests(unittest.TestCase):
    def test_2d_and_3d_shared_cd_ticks_use_expected_divisions(self):
        ticks = _axis_ticks(pd.Series([0.31, 0.51]))
        self.assertIn(0.325, ticks.tolist())
        self.assertIn(0.350, ticks.tolist())
        self.assertIn(0.375, ticks.tolist())
        self.assertTrue(all(
            abs((right - left) - 0.025) < 1e-12
            for left, right in zip(ticks[:-1], ticks[1:])
        ))

    def test_marker_area_shrinks_for_large_experiments(self):
        small = adaptive_marker_area(80, dimensions=2)
        medium = adaptive_marker_area(800, dimensions=2)
        large = adaptive_marker_area(5000, dimensions=2)
        self.assertGreater(small, medium)
        self.assertGreater(medium, large)
        self.assertLessEqual(large, 1.0)
        self.assertLess(
            adaptive_marker_area(5000, dimensions=3),
            adaptive_marker_area(5000, dimensions=2),
        )

    def test_global_selection_prefers_continuous_daylight_score(self):
        data = _frame([
            {"material": "A", "Ra": 0.30, "daylight_score": 0.62,
             "thermal_discomfort": 50.0, "cost": 100.0},
            {"material": "B", "Ra": 0.30, "daylight_score": 0.68,
             "thermal_discomfort": 50.0, "cost": 100.0},
        ])
        front = global_pareto_front(data, u0_min=0.0)
