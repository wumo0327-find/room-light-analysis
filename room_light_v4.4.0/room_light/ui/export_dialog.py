"""
ui/export_dialog.py — 勾选式导出对话框  v2.4.0
"""
...
class ExportDialog(QDialog):
    def __init__(self, has_daylight: bool, has_thermal: bool,
                 has_experiment: bool = False, has_optimal: bool = False,
                 parent=None):
        ...
        # ── 参数化实验组 ──────────────────────────────────────────────────
        grp_exp = QGroupBox("参数化实验与最优方案")
        exp_lay = QVBoxLayout(grp_exp)
        self.chk_exp_plots = QCheckBox("参数化实验4张图（3D＋三个二维投影）")
        self.chk_exp_csv = QCheckBox("参数化实验完整CSV（含造价分项与筛选标记）")
        self.chk_optimal_plots = QCheckBox(
            "最优方案完整分析图（采光热力图/截面图/热环境/光热综合）")
        self.chk_exp_plots.setChecked(has_experiment)
        self.chk_exp_csv.setChecked(has_experiment)
        self.chk_optimal_plots.setChecked(has_optimal)
        self.chk_exp_plots.setEnabled(has_experiment)
        self.chk_exp_csv.setEnabled(has_experiment)
        self.chk_optimal_plots.setEnabled(has_optimal)
        for chk in (self.chk_exp_plots, self.chk_exp_csv, self.chk_optimal_plots):
            exp_lay.addWidget(chk)
        ...
        self.chk_xls = QCheckBox(
            "导出 Excel（含常规分析、参数实验、造价分项及最优方案数据）")
        has_any = has_daylight or has_thermal or has_experiment or has_optimal
        self.chk_xls.setChecked(has_any)
        self.chk_xls.setEnabled(has_any)
        ...
        self._prefix_edit = QLineEdit("建筑光热分析结果")
    ...
    def _select_all(self):
        for chk in (self.chk_heatmap, self.chk_profile,
                    self.chk_thermal, self.chk_combined,
                    self.chk_exp_plots, self.chk_exp_csv,
                    self.chk_optimal_plots, self.chk_xls):
    ...
    def export_prefix(self) -> str:
        return getattr(self, "_export_prefix", "建筑光热分析结果")
