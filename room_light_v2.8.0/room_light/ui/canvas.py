...
from core.plan_export import (
    ...
    C_DIM, C_COMPASS, dim_arrow as _plan_dim_arrow,
...
class RoomCanvas(QWidget):
    ...
    # ── Floor-plan view ───────────────────────────────────────────────────────
    # v2.8.0: 绘制逻辑已抽取到 core/plan_export.py（GUI 画布与命令行静态导出共用），
    # 本方法只是薄封装，行为与旧版逐像素一致。
    ...
    # ── Helpers ───────────────────────────────────────────────────────────────
    # v2.8.0: 委托给 core/plan_export.py 的共享实现（elevation 视图仍复用这两个）。
