...
class DraftCanvas(QWidget):
    ...
    def _wall_grip_target(self, raw: Point) -> Point:
        ...
        if not hosted:
            return self._drawing_snap(raw)
        # A hosted window must stay on the same physical axis.  Snap the
        # cursor's longitudinal coordinate to the drawing grid while allowing
        # extension beyond the current finite segment.
        dx, dy = wall.direction
        along = (
            (raw.x - wall.start.x) * dx
            + (raw.y - wall.start.y) * dy
        )
        spacing = max(float(self.document.grid_mm), 1.0)
        along = round(along / spacing) * spacing
