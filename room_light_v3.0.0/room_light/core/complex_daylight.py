"""
core/complex_daylight.py — Arbitrary-polygon daylight engine  v3.0.0
...
from core.complex_models import Point2D, SpaceModel, WallOpening, WallSegment
...
def _ds_point(
    ...
    contribution = np.where(
        valid,
        sky_luminance * cos_workplane * solid_angle,
        0.0,
    )
...
def _dext_point(
    ...
    _normal, u_axis, v_axis, origin = _wall_axes_m(wall)
    sill_u = (
...
def compute_complex_daylight(
    ...
    result.method = (
        ...
        f"+ arbitrary polygon occlusion (ndiv={ndiv})"
    )
