...
def export_excel(
    ...
    finite_e = E_arr[np.isfinite(E_arr)]
    max_e = float(finite_e.max()) if finite_e.size else 1
...
def export_excel_v2(
    ...
    if daylight_result and daylight_result.E_lux is not None:
        ...
        ws3.conditional_formatting.add(
            ...
            ColorScaleRule(start_type="num",start_value=0,start_color="FFffffff",
                           mid_type="num",mid_value=300,mid_color="FF86efac",
                           end_type="num",
                           end_value=int(np.nanmax(E)) if np.isfinite(E).any() else 1,
                           end_color="FFef4444"))
        for col in range(1,len(xs)+2):
            ws3.column_dimensions[get_column_letter(col)].width=8
    ...
    if has_experiment:
        ...
        info_rows = [
            ("工程关联",
             "实验采用当前.rlproj的房间尺寸、窗户尺寸/位置、玻璃光学与热工参数，以及当前气候数据。"),
            ("改造前基准",
             "完整保留导入.rlproj当前状态（包括已有水平/垂直遮阳）计算一次，"
             "用于改造前后对比；新增改造造价记为0元。"),
            ("L=0规则",
             "新方案先移除原遮阳；L=0表示无水平/垂直遮阳，不随倾角θ、间隙h"
             "或材料重复计算。若原工程本来无遮阳，同一个基准点兼任L=0候选。"),
            ("造价口径",
             "板材面积=全部窗宽合计×板长L；支撑长度=2×全部窗宽合计；"
             "总价=板材面积×材料综合单价+支撑长度×支撑单价+窗数×安装费。"),
            ("造价性质",
        ...
        if experiment_params:
            info_rows.extend([
                ...
                ("支撑综合单价(元/m)",
                 float(experiment_params.get("support_cost_per_m", 180.0))),
                ("单窗安装费(元/窗)",
                 float(experiment_params.get("install_cost_per_window", 300.0))),
                ("U₀筛选下限",
    ...
    if has_optimal:
        ...
        ws7.cell(1, 1, "参数化实验均衡推荐方案").font = _font(True, _A, 14)
        ws7.cell(2, 1, optimal_label or "均衡推荐方案").font = _font(True, _S, 10)
        _hrow(ws7, 4, ["项目", "数值"])
        summary_rows = []
        if has_experiment and "balanced_recommended" in experiment_df.columns:
            selected = experiment_df[
                experiment_df["balanced_recommended"].astype(bool)]
        ...
        if optimal_thermal_result is not None:
            summary_rows.extend([
                ...
                ("最优方案舒适月数", optimal_thermal_result.comfort_months),
                ("最优方案超温月数", optimal_thermal_result.overheat_months),
                ("最优方案欠温月数", optimal_thermal_result.underheat_months),
                ("最优方案SC_effective",
        ...
        if (optimal_daylight_result is not None
            ...
            for r_idx, y in enumerate(ys, 2):
                ...
                for c_idx, value in enumerate(E[r_idx - 2], 2):
                    _dcel(ws9, r_idx, c_idx, round(float(value)))
            for col in range(1, len(xs) + 2):
