"""
ui/experiment_sidebar.py — 参数化实验专用侧边栏  v3.0.0
================================================================
    ...
    def set_complex_project_context(
        ...
        if space is None:
            ...
            detail = (
                ...
                f"外窗 {len(windows)}扇；玻璃τ={tau_text}"
            )
        self._project_has_shading = False
        self._project_context.setText(
            f"{name}\n{detail}；气象={wx or '默认'}\n"
            "复杂空间参数化实验正在迁移，本页参数可查看，但暂不运行旧矩形引擎。"
        )
        self._btn_run.setEnabled(False)
        self._btn_run.setToolTip(
            "复杂空间参数化实验尚未迁移完成，当前禁止调用旧矩形引擎。"
        )
