...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("建筑室内采光分析工具  v3.0.0  |  益阳默认气象")
        self.resize(1200, 760)   # v2.12.0: 默认非全屏窗口调小（原1480×900）
        self.setMinimumSize(1080, 700)
        self.setAcceptDrops(True)

        self.room    = RoomModel()
        self.building = building_from_room(self.room)
        self._active_space_id: str | None = "legacy_space"
        self._complex_mode = True
        ...
        # v3: “建筑视图”统一使用复杂空间模型侧栏。参数化实验入口保留，
        # 但迁移完成前不会调用旧矩形计算引擎。
    ...
    def _update_title(self):
        ...
        suf = f"  —  {fn}" if fn else ""
        self.setWindowTitle(f"建筑室内采光分析工具  v3.0.0{suf}")
    ...
    def _on_step_done(self, idx: int, name: str, result):
        if "采光" in name and result is not None:
            ...
            display_model = (
                self.building.get_space(self._active_space_id)
                if self._complex_mode and self._active_space_id
                else self.room
            )
    ...
    def _run_experiment(self, params: dict):
        if self._complex_mode:
            QMessageBox.information(
                self,
                "复杂空间参数化实验正在迁移",
                "当前建筑视图已使用复杂空间模型。为避免得到错误结果，"
                "程序不会调用旧矩形参数化引擎；本阶段仍可查看参数页，"
                "后续将把遮阳扫描迁移到逐墙复杂空间计算。",
            )
            return
        if not self.room.windows:
