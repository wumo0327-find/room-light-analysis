"""
core/complex_daylight.py — Arbitrary-polygon daylight engine  v4.2.1
...
def _ds_point(
    ...
    overhang_blocked = np.zeros_like(qx, dtype=bool)
    ...
    direct_contribution = np.where(overhang_blocked, 0.0, base_contribution)
    blocked_contribution = np.where(overhang_blocked, base_contribution, 0.0)
...
def compute_complex_daylight(
    ...
    result.method = (
        ...
        f"+ conservative overhang first-bounce reflection "
        f"(ndiv={ndiv})"
    )
