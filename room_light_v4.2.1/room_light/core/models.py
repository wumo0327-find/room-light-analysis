"""
core/models.py — Legacy rectangular data models  v4.2.1
...
class ShadingDevice:
    ...
    # 关于斜面射线相交判定的推导；v4.2.1起热工SC使用倾斜剖面阴影式，仍以
    ...
    # v4.2.1: 遮阳板表面光热属性。旧工程缺少这些字段时使用保守默认值，
    ...
    def beam_shade_fraction(self, profile_angle_deg, window_height_mm: float,
        ...
        tilt_deg：θ=90°水平（默认）。v4.2.1起按太阳剖面内的板尖水平和竖向位移
...
# v4.2.1 光热属性补全。保留 ``reflect`` 旧字段作为论文既有数据口径，
