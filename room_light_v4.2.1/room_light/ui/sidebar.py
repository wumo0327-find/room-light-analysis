"""
ui/sidebar.py — 可折叠参数侧边栏  v2.10.2
BUG FIX:
...
class Sidebar(QWidget):
    ...
    def _build_shading_section(self):
        ...
        # v2.10.2 新增：柱体真实宽度（仅用于两端外墙位置；窗间墙位置的柱子
        # 仍按真实窗间墙宽度，不受此参数影响，见 core/daylight.py 说明）
        self._sh_fin_colw = _dspin(s.fin_column_width_mm, 0, 3000, 10, 0)
        sec.add_row("端墙柱宽", self._sh_fin_colw, "mm")

        hint = _lbl("挑檐仅对南向窗按「月中日正午」剖面角逐月遮阳；翼板每扇窗左右两侧\n"
                    "各一处（最左/最右窗对应端墙，其余对应窗间墙），可在下方逐窗覆盖。\n"
                    "端墙柱宽=贴窗边缘的柱子实际宽度（不是整段端墙剩余宽度，柱子之外\n"
                    "仍是普通墙体不参与遮挡）；窗间墙柱宽固定等于真实窗间墙宽度。\n"
                    "改参数后点工具栏「▶ 分析选中房间」刷新结果。",
        ...
        self._sh_fin_colw.valueChanged.connect(
            lambda v: self._on_shade_p("fin_column_width_mm", v))
