"""Building-plan drafting main window, reusable standalone or embedded."""
...
class MainWindow(QMainWindow):
    embedded_save_requested = pyqtSignal()

    def __init__(self, parent=None, *, embedded_mode: bool = False):
        super().__init__(parent)
        self.embedded_mode = bool(embedded_mode)
    ...
    def _build_toolbar(self) -> None:
        ...
        actions = [
            ...
            (
                "保存并同步" if self.embedded_mode else "保存",
                self._save_or_sync,
        ...
        if not self.embedded_mode:
            actions.insert(4, ("导出rlproj", self.export_rlproj_project, None))
        ...
        self.osnap_action = QAction("F3 对象捕捉", self)
        self.osnap_action.setShortcut("F3")
        self.osnap_action.setCheckable(True)
        self.osnap_action.setChecked(True)
        self.osnap_action.setToolTip(
            "切换CAD式对象捕捉：端点、中点、交点、垂足、"
            "最近点、墙体边线（F3）"
        )
        self.osnap_action.toggled.connect(self._set_object_snap)
        toolbar.addAction(self.osnap_action)

    def _save_or_sync(self) -> None:
        """Embedded Ctrl+S commits the BP draft to its RoomLight host."""
        if self.embedded_mode:
            if self.current_file is not None:
                try:
                    ...
                    return
                self._show_prompt(f"BP源文件已保存：{self.current_file.name}")
            self.embedded_save_requested.emit()
            return
        self.save_document()
    ...
    def _build_central(self) -> None:
        ...
        self.osnap_label = QLabel("对象捕捉 F3：开")
        self.osnap_label.setObjectName("modeBadge")
        info_layout.addWidget(title)
        info_layout.addStretch()
        info_layout.addWidget(self.counts)
        info_layout.addWidget(self.ortho_label)
        info_layout.addWidget(self.osnap_label)
        root.addWidget(info)
    ...
    def _execute_command(self) -> None:
        ...
        if command == "F8":
            self._toggle_ortho()
            return
        if command in {"F3", "OSNAP"}:
            self.osnap_action.setChecked(
                not self.osnap_action.isChecked()
            )
            return
    ...
    def _set_object_snap(self, enabled: bool) -> None:
        self.canvas.set_object_snap_enabled(enabled)
        self.osnap_label.setText(
            f"对象捕捉 F3：{'开' if enabled else '关'}"
        )
        self.canvas.setFocus()
    ...
    def eventFilter(self, watched, event) -> bool:
        ...
        # When embedded in RoomLight, the host QDialog itself is modal.  It is
        # still the editor's own window, so CAD-style global command typing
        # must remain active.  Other modal dialogs (file pickers/messages)
        # continue to receive their keys normally.
        if active_modal is not None and active_modal is not self.window():
            return super().eventFilter(watched, event)
