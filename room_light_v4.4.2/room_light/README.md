...
v4.4.2 修正改造前基准反复成为推荐方案的问题：只要存在年运行费用低于基准的真实改造方案，L0基准即退出最终推荐池，程序在节能改造方案中继续按硬约束、最小干预层级和残余差距选优；同时导出年运行节省额、节省率和静态回收期。

当前v4.4.2还将常规采光分析网格细化为150mm，并让最外圈网格单元贴合实体墙内边界，不再保留固定500mm白带；采光热力图按模型毫米数据画出真实墙厚、窗洞与凹窗位置，热环境图表增加次级网格线。参数化大批量初筛保留250mm网格，最终推荐方案再用150mm复算。
