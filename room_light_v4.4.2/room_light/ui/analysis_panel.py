"""
ui/analysis_panel.py — 采光分析热力图面板  v4.4.2
修复:
...
from core.complex_models import Point2D, SpaceModel
...
def _segment_strip_vertices(start, end, width_mm, square_caps=True):
    """Return a real-width plan strip in model millimetres."""
    ...
    nx, ny = -uy, ux
    half = max(1.0, float(width_mm)) / 2.0
    cap = half if square_caps else 0.0
    sx, sy = float(start.x) - ux * cap, float(start.y) - uy * cap
    ex, ey = float(end.x) + ux * cap, float(end.y) + uy * cap
    return [
...
def _add_strip(ax, start, end, width_mm, *, facecolor, edgecolor,
               linewidth, zorder, square_caps=True, alpha=1.0):
    vertices = _segment_strip_vertices(
        start, end, width_mm, square_caps=square_caps)
    if not vertices:
        return None
    patch = patches.Polygon(
        vertices, closed=True, facecolor=facecolor, edgecolor=edgecolor,
...
def _model_bounds(room: RoomModel | SpaceModel):
    if isinstance(room, SpaceModel):
        ...
        for loop in room.boundary_loops:
            for wall in loop.segments:
                vertices.extend(_segment_strip_vertices(
                    wall.start, wall.end, wall.thickness_mm))
                outward_x, outward_y = wall.outward_normal
                for opening in wall.windows():
                    offset = max(0.0, float(opening.plane_offset_mm))
                    host_start = wall.point_at(opening.offset_mm)
                    host_end = wall.point_at(opening.end_offset_mm)
                    glazing_start = Point2D(
                        host_start.x + outward_x * offset,
                        host_start.y + outward_y * offset)
                    glazing_end = Point2D(
                        host_end.x + outward_x * offset,
                        host_end.y + outward_y * offset)
                    vertices.extend(_segment_strip_vertices(
                        glazing_start, glazing_end, 60.0,
                        square_caps=False))
        if not vertices:
    ...
    thickness = max(1.0, float(room.thermal.wall_thickness_mm))
    return (
        -thickness, float(room.width) + thickness,
        -thickness, float(room.length) + thickness,
    )
...
def _draw_model_outline_and_windows(ax, room, width_mm, length_mm):
    if isinstance(room, SpaceModel):
        for loop in room.boundary_loops:
            for wall in loop.segments:
                ...
                _add_strip(
                    ax, wall.start, wall.end, wall_width,
                ...
                for opening in wall.windows():
                    ...
                    host_end = wall.point_at(opening.end_offset_mm)
                    _add_strip(
                        ax, host_start, host_end, wall_width * 1.08,
                        facecolor=C_BG, edgecolor=C_BG, linewidth=0.0,
                        zorder=6, square_caps=False)
                    outward_x, outward_y = wall.outward_normal
                    offset = max(0.0, float(opening.plane_offset_mm))
                    glazing_start = Point2D(
                        host_start.x + outward_x * offset,
                    ...
                    glazing_end = Point2D(
                        host_end.x + outward_x * offset,
                        host_end.y + outward_y * offset)
                    glazing_width = max(24.0, min(60.0, wall_width * 0.20))
                    _add_strip(
                        ax, glazing_start, glazing_end, glazing_width,
                        facecolor="#67e8f9", edgecolor="#0284c7",
                        linewidth=1.0, zorder=7, square_caps=False)
                    if offset > 0.0:
                        for host, glass in (
                            (host_start, glazing_start),
                            (host_end, glazing_end),
                        ):
                            ax.plot(
                                [host.x, glass.x], [host.y, glass.y],
                                color="#94a3b8", linewidth=0.75,
                                linestyle=":", zorder=6.5,
                            )
        return
    ...
    wall_style = dict(
        facecolor="#e5e7eb", edgecolor="#64748b",
        linewidth=0.85, zorder=5)
    ax.add_patch(patches.Rectangle((0, -thickness), width_mm, thickness,
                                   **wall_style))
    ax.add_patch(patches.Rectangle((0, length_mm), width_mm, thickness,
                                   **wall_style))
    ax.add_patch(patches.Rectangle((-thickness, 0), thickness, length_mm,
                                   **wall_style))
    ax.add_patch(patches.Rectangle((width_mm, 0), thickness, length_mm,
                                   **wall_style))
    for window in room.windows:
        wall = window.wall
        if wall == "south":
            bx, by, bw, bh = window.x, -thickness, window.width, thickness
        elif wall == "north":
            bx, by, bw, bh = (
                window.x, length_mm, window.width, thickness
            )
        elif wall == "east":
            bx, by, bw, bh = (
                width_mm, window.x, thickness, window.width
            )
        else:
            bx, by, bw, bh = -thickness, window.x, thickness, window.width
...
class AnalysisPanel(QWidget):
    ...
    def _build_control_bar(self) -> QWidget:
        ...
        hint = QLabel("精度: WIN_DIV=40 · 常规分析网格=150mm")
        hint.setStyleSheet(
    ...
    def _render_heatmap_ax(
        ...
        pcm = ax_m.pcolormesh(
            XE, YE, E, cmap=cmap, norm=norm, shading="flat", zorder=2,
            rasterized=True, edgecolors=(1.0, 1.0, 1.0, 0.20),
            linewidth=0.16)
        ...
        ax_m.xaxis.set_major_locator(mticker.MultipleLocator(1000.0))
        ax_m.yaxis.set_major_locator(mticker.MultipleLocator(1000.0))
        ax_m.xaxis.set_minor_locator(mticker.MultipleLocator(500.0))
        ax_m.yaxis.set_minor_locator(mticker.MultipleLocator(500.0))
        ax_m.grid(True, which="major", color="#dbe2ea", linewidth=0.45,
                  alpha=0.75, zorder=3.4)
        ax_m.grid(True, which="minor", color="#edf1f5", linewidth=0.32,
                  alpha=0.70, zorder=3.3)
    ...
    def _render_profile_ax(self, ax):
        ...
        ax.grid(True, which="major", color=C_GRID, linewidth=0.55, alpha=0.9)
        ax.grid(True, which="minor", color="#f4f6f9", linewidth=0.35, alpha=0.9)
