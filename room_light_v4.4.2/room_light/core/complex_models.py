"""
core/complex_models.py — Complex building/space data models  v4.4.2
...
class WallSegment:
    ...
    @property
    def analysis_length_mm(self) -> float:
        """Net inner-face length used for envelope area when available."""
        try:
            value = float(self.metadata.get("analysis_length_mm", self.length_mm))
        except (TypeError, ValueError):
            value = self.length_mm
        return value if math.isfinite(value) and value >= 0.0 else self.length_mm
...
@dataclass
class FloorLoop:
    """Net usable-floor boundary independent from wall/opening host axes.

    BP walls are authored on configurable centre/left/right axes.  Windows
    must remain hosted on those axes, while work-plane points and floor area
    must follow the physical inner wall faces.  Keeping this loop separate
    avoids moving or clipping corner windows merely to obtain a net room area.
    Older projects omit it and continue to use ``boundary_loops``.
    """

    points: List[Point2D] = field(default_factory=list)
    kind: str = "outer"
    id: str = field(default_factory=lambda: new_id("floor_loop"))
    metadata: Dict[str, object] = field(default_factory=dict)
...
class SpaceModel:
    ...
    roof_exposed: bool = True
    floor_exposed: bool = True
    boundary_loops: List[BoundaryLoop] = field(default_factory=list)
    analysis_floor_loops: List[FloorLoop] = field(default_factory=list)
    ...
    def analysis_outer_points(self) -> List[Point2D]:
        loop = next(
            (item for item in self.analysis_floor_loops if item.kind == "outer"),
            None,
        )
        if loop is not None and len(loop.points) >= 3:
            return list(loop.points)
        outer = self.outer_loop()
        return list(outer.points()) if outer is not None else []

    def analysis_hole_points(self) -> List[List[Point2D]]:
        loops = [
            list(item.points)
            for item in self.analysis_floor_loops
            if item.kind == "hole" and len(item.points) >= 3
        ]
        if loops:
            return loops
        return [list(loop.points()) for loop in self.hole_loops()]
