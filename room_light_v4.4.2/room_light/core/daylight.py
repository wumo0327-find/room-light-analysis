...
WALL_MARGIN_MM = 0.0      # 最外圈网格单元贴合墙体内边界
...
def _ds_point(P: np.ndarray, win, room: RoomModel, ndiv: int = WIN_DIV) -> float:
    ...
    applies = sh.applies_to(win.id)
    if applies and sh.has_component("horizontal_overhang") and over_depth_mm > 0.0:
    ...
    if applies and sh.has_component("louver"):
        horizontal = np.maximum(np.sqrt(dX * dX + dY * dY), 1e-12)
        profile = np.degrees(np.arctan2(np.maximum(dZ, 0.0), horizontal))
        blockage = np.clip(
            max(sh.louver_width_mm, 0.0)
            * np.abs(np.sin(math.radians(sh.louver_angle_deg) - np.radians(profile)))
            / max(sh.louver_spacing_mm, 1e-6), 0.0, 1.0,
        )
        position_factor = {"exterior": 1.0, "between": 0.88, "interior": 0.72}.get(
            sh.louver_position, 1.0
        )
        reflected = 0.10 * float(np.clip(sh.louver_reflect, 0.0, 1.0))
        overhang_transmission *= (
            1.0 - blockage * position_factor * (1.0 - reflected)
        )

    if applies and sh.has_component("light_shelf"):
        ratio = float(np.clip(sh.light_shelf_upper_window_ratio, 0.05, 0.90))
        shelf_z = sh.light_shelf_height_mm / 1000.0
        if shelf_z <= 0.0:
            shelf_z = (win.y + win.height * (1.0 - ratio)) / 1000.0
        inside = max(sh.light_shelf_inside_depth_mm, 0.0) / 1000.0
        outside = sh.effective_shelf_outside_depth_mm() / 1000.0
        shelf_tan = math.tan(math.radians(sh.light_shelf_tilt_deg))
        point_inward = (P[0] - orig[0]) * norm[0] + (P[1] - orig[1]) * norm[1]
        ray_inward = dX * norm[0] + dY * norm[1]
        with np.errstate(divide="ignore", invalid="ignore"):
            ts = (shelf_z + point_inward * shelf_tan - P[2]) / (
                dZ - ray_inward * shelf_tan
            )
        sx, sy = P[0] + ts * dX, P[1] + ts * dY
        local_u = (sx - orig[0]) * uax[0] + (sy - orig[1]) * uax[1]
        inward = (sx - orig[0]) * norm[0] + (sy - orig[1]) * norm[1]
        lateral_ok = (local_u >= win.x / 1000.0) & (
            local_u <= (win.x + win.width) / 1000.0
        )
        blocked_shelf = np.zeros_like(Qx, dtype=bool)
        if inside > 0.0:
            blocked_shelf |= (ts > 0.0) & (ts < 1.0) & (inward >= 0.0) & (
                inward <= inside
            ) & lateral_ok
        if outside > 0.0:
            blocked_shelf |= (ts > 1.0) & (inward <= 0.0) & (
                -inward <= outside
            ) & lateral_ok
        shelf_return = 0.12 * float(np.clip(sh.light_shelf_reflect, 0.0, 1.0))
        overhang_transmission = np.where(
            blocked_shelf, shelf_return, overhang_transmission
        )
...
def compute(
    ...
    # 均匀单元中心：首末单元边缘精确落在墙体内边界，不再保留500 mm白带。
    def axis_centres(low, high):
    ...
    xs = axis_centres(0.0, W)
    ys = axis_centres(0.0, L)
    nx, ny = len(xs), len(ys)
    res.grid_x = xs; res.grid_y = ys
