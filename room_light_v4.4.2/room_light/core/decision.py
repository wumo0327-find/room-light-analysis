"""
core/decision.py — 可行域、残余差距与最小干预判定  v4.4.2
...
def annotate_minimum_intervention(
    ...
    不再参与最终推荐。程序只在这些节能改造方案中继续执行硬约束、最小层级和
    残余差距判断。这里比较的是 annual_operating_cost，不含改造建设费用。
    ...
    feasible = selection_pool[selection_pool["feasible"].astype(bool)].copy()
    ...
    if not feasible.empty:
        ...
        if energy_saving_pool_active:
            ...
            out.at[chosen_index, "decision_kind"] = "节能改造最低层级可行方案"
            out.at[chosen_index, "decision_reason"] = (
                f"已发现年运行费用低于改造前基准（{baseline_cost:.2f}元/年）的改造方案，"
                "因此L0基准仅作参照；"
                f"本方案运行费用为{chosen_cost:.2f}元/年，年节省{saving_value:.2f}元"
                f"{payback_text}。{level}已满足全部硬约束，并按“{criterion}”在该层级内最小。"
            )
    else:
        ...
        if energy_saving_pool_active:
            ...
            out.at[chosen_index, "decision_kind"] = "节能改造最小残余差距"
            out.at[chosen_index, "decision_reason"] = (
                f"已发现年运行费用低于改造前基准（{baseline_cost:.2f}元/年）的改造方案，"
                "因此L0基准仅作参照；"
                f"本方案运行费用为{chosen_cost:.2f}元/年，年节省{saving_value:.2f}元"
                f"{payback_text}。当前参数域仍无方案满足全部硬约束，故在节能改造方案中"
                "选择最大归一化违反度最小者，并保留主动补偿提示。"
            )
