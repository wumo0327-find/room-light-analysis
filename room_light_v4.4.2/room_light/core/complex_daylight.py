"""
core/complex_daylight.py — Arbitrary-polygon daylight engine  v4.4.2
...
GRID_MM = 250.0
WIN_DIV = 40
# 工作面覆盖到墙体内边界。采样点位于最外圈网格单元中心，
# 不会落到墙体上；绘图单元边缘则恰好贴住墙内边线。
WALL_MARGIN_MM = 0.0
...
class ComplexDaylightResult:
    ...
    def __init__(self):
        ...
        self.ndiv = None
        self.Ra = None
        self.daylight_score = None
        self.Ra_threshold = None
        self.geometry_basis = None
        self.net_floor_area_m2 = None
...
def _distance_to_edge(point: Point2D, start: Point2D, end: Point2D) -> float:
    dx = end.x - start.x
    dy = end.y - start.y
    length2 = dx * dx + dy * dy
    if length2 <= 1e-18:
    ...
    return point.distance_to(Point2D(start.x + t * dx, start.y + t * dy))
...
def build_workplane_grid(
    ...
    """
    points = space.analysis_outer_points()
    if not points:
    ...
    margin = max(0.0, float(wall_margin_mm))

    def axis_centres(low: float, high: float) -> np.ndarray:
        """Return uniform cell centres whose outside edges equal low/high."""
        inner_low = low + margin
        inner_high = high - margin
        if inner_high <= inner_low:
            inner_low, inner_high = low, high
        span = max(inner_high - inner_low, 1e-9)
        count = max(1, int(math.ceil(span / grid_mm)))
        cell = span / count
        return inner_low + (np.arange(count, dtype=float) + 0.5) * cell

    xs = axis_centres(min_x, max_x)
    ys = axis_centres(min_y, max_y)
    nx, ny = len(xs), len(ys)
    floor_rings = [points] + space.analysis_hole_points()
    floor_edges = [
        (start, ring[(index + 1) % len(ring)])
        for ring in floor_rings if len(ring) >= 3
        ...
        for iy, y in enumerate(ys):
            for ix, x in enumerate(xs):
                ...
                if not point_in_space(point, space, include_boundary=False):
                    continue
                if require_margin and margin > 0.0 and floor_edges:
                    if min(
                        _distance_to_edge(point, start, end)
...
def _space_rho_bar(space: SpaceModel) -> float:
    ...
    wall_gross = sum(
        wall.analysis_length_mm * space.height_mm / 1_000_000.0
...
def _blocked_by_other_boundaries(
    ...
    for wall in walls:
        ...
        intersects = (
            ...
            & (t > 1e-9)
            # t=1 is the glazing sample itself.  BP models commonly use a
            # full-width window whose two ends coincide with return walls;
            # touching those walls at the window plane must not count as an
            # obstruction, while any crossing strictly before it still does.
            & (t < 1.0 - 1e-9)
            & (u >= -1e-9)
            & (u <= 1.0 + 1e-9)
        )
...
def _overhang_reflection_fraction(shading) -> float:
    """Conservative first-bounce return from any intercepted passive device."""
    rho = float(np.clip(
        getattr(shading, "visible_reflectance", 0.32), 0.0, 1.0
    ))
    if getattr(shading, "has_component", lambda _kind: False)("louver"):
        rho = max(rho, float(np.clip(
            getattr(shading, "louver_reflect", rho), 0.0, 1.0
        )))
    if getattr(shading, "has_component", lambda _kind: False)("light_shelf"):
        rho = max(rho, float(np.clip(
            getattr(shading, "light_shelf_reflect", rho), 0.0, 1.0
        )))
    specular = float(np.clip(
        getattr(shading, "specular_fraction", 0.03), 0.0, 1.0
    ))
...
def _ds_point(
    ...
    device_transmission = np.ones_like(qx, dtype=np.float64)

    shading = space.shading
    shading_key = str(
        opening.metadata.get("legacy_window_id", opening.id)
    )
    applies = getattr(shading, "applies_to", lambda _id: True)(shading_key)
    depth_mm, gap_mm, tilt_deg = shading.get_overhang_for(shading_key)
    if (
        applies
        and getattr(shading, "has_component", lambda kind: (
            getattr(shading, "type", "none") == kind
        ))("horizontal_overhang")
        and depth_mm > 0.0
    ):
        ...
        if abs(sin_theta) > 1e-9:
            ...
            device_transmission = np.where(blocked, 0.0, device_transmission)

    # v4.4.0 百叶：以周期叶片在射线剖面上的投影开口率计算透过，不再把百叶
    # 简化成一个固定SC。每个窗面元随视线仰角得到连续值，因此角度/片宽/间距
    # 会同时改变采光分布。季节控制在静态采光图中按“展开态”展示。
    if applies and getattr(shading, "has_component", lambda _kind: False)("louver"):
        horizontal = np.maximum(
            np.sqrt(delta_x * delta_x + delta_y * delta_y), 1e-12
        )
        profile = np.degrees(np.arctan2(np.maximum(delta_z, 0.0), horizontal))
        spacing = max(float(shading.louver_spacing_mm), 1e-6)
        width_mm = max(float(shading.louver_width_mm), 0.0)
        theta = math.radians(float(shading.louver_angle_deg))
        blockage = np.clip(
            width_mm * np.abs(np.sin(theta - np.radians(profile))) / spacing,
            0.0, 1.0,
        )
        position_factor = {
            "exterior": 1.0, "between": 0.88, "interior": 0.72,
        }.get(str(getattr(shading, "louver_position", "exterior")), 1.0)
        device_transmission *= 1.0 - blockage * position_factor

    # v4.4.0 反光板：按真实窗局部坐标求射线与内/外水平板的交点。内板位于
    # 观察点和窗之间(t<1)，外板位于窗外(t>1)。被截获的天空光进入一次反射
    # 基底，后续按板面反射率重组；这能体现深进深补光与近窗遮挡的权衡。
    if applies and getattr(shading, "has_component", lambda _kind: False)("light_shelf"):
        ratio = float(np.clip(
            getattr(shading, "light_shelf_upper_window_ratio", 0.35),
            0.05, 0.90,
        ))
        shelf_z = float(getattr(shading, "light_shelf_height_mm", 0.0)) / 1000.0
        if shelf_z <= 0.0:
            shelf_z = (
                opening.sill_height_mm
                + opening.height_mm * (1.0 - ratio)
            ) / 1000.0
        inside = max(float(getattr(
            shading, "light_shelf_inside_depth_mm", 0.0
        )) / 1000.0, 0.0)
        outside = max(float(getattr(
            shading, "effective_shelf_outside_depth_mm", lambda: 0.0
        )()) / 1000.0, 0.0)
        shelf_tan = math.tan(math.radians(float(getattr(
            shading, "light_shelf_tilt_deg", 0.0
        ))))
        point_inward = (
            (point[0] - origin[0]) * normal[0]
            + (point[1] - origin[1]) * normal[1]
        )
        ray_inward = delta_x * normal[0] + delta_y * normal[1]
        with np.errstate(divide="ignore", invalid="ignore"):
            t_shelf = (
                shelf_z + point_inward * shelf_tan - point[2]
            ) / (delta_z - ray_inward * shelf_tan)
        sx = point[0] + t_shelf * delta_x
        sy = point[1] + t_shelf * delta_y
        local_u = (sx - origin[0]) * u_axis[0] + (sy - origin[1]) * u_axis[1]
        inward = (sx - origin[0]) * normal[0] + (sy - origin[1]) * normal[1]
        lateral_ok = (local_u >= u0 - 1e-9) & (local_u <= u0 + width + 1e-9)
        shelf_blocked = np.zeros_like(qx, dtype=bool)
        if inside > 0.0:
            shelf_blocked |= (
                np.isfinite(t_shelf) & (t_shelf > 1e-9) & (t_shelf < 1.0 - 1e-9)
                & (inward >= -1e-9) & (inward <= inside + 1e-9) & lateral_ok
            )
        if outside > 0.0:
            shelf_blocked |= (
                np.isfinite(t_shelf) & (t_shelf > 1.0 + 1e-9)
                & (inward <= 1e-9) & (-inward <= outside + 1e-9) & lateral_ok
            )
        device_transmission = np.where(shelf_blocked, 0.0, device_transmission)
    ...
    base_contribution = np.where(
        valid,
        sky_luminance * cos_workplane * solid_angle * barrier_transmission,
        0.0,
    )
    device_transmission = np.clip(device_transmission, 0.0, 1.0)
    direct_contribution = base_contribution * device_transmission
    blocked_contribution = base_contribution * (1.0 - device_transmission)
    outdoor_normalisation = 7.0 * math.pi / 9.0
    scale = opening.visible_transmittance / (math.pi * outdoor_normalisation)
    direct = float(max(0.0, direct_contribution.sum() * scale * 100.0))
    blocked_basis = float(max(0.0, blocked_contribution.sum() * scale * 100.0))
    if return_components:
        return direct, blocked_basis
    return direct + _overhang_reflection_fraction(shading) * blocked_basis
...
def compute_complex_daylight(
    ...
    result.method = (
        ...
        f"+ physical inner-face work-plane boundary "
        f"+ {len(space.exterior_barriers)} explicit daylight barriers "
        f"+ overhang/louver/light-shelf ray attenuation and first-bounce reflection "
        f"(ndiv={ndiv})"
    )
    ...
    ds_direct_array = np.full(shape, np.nan, dtype=np.float64)
    ds_blocked_array = np.full(shape, np.nan, dtype=np.float64)
    ...
    for iy, y in enumerate(ys):
        ...
        for ix, x in enumerate(xs):
            ...
            ds_total = 0.0
            ds_direct_total = 0.0
            ds_blocked_total = 0.0
            dext_total = 0.0
            for wall, opening in windows:
                direct, blocked_basis = _ds_point(
                    point, wall, opening, space, ndiv,
                    return_components=True,
                )
                ds_direct_total += direct
                ds_blocked_total += blocked_basis
                dext_total += _dext_point(point, wall, opening, space)
            ds_total = (
                ds_direct_total
                + _overhang_reflection_fraction(space.shading)
                * ds_blocked_total
            )
            dint = _dint(ds_total, dext_total, rho)
            ds_array[iy, ix] = ds_total
            ds_direct_array[iy, ix] = ds_direct_total
            ds_blocked_array[iy, ix] = ds_blocked_total
    ...
    result.Ds_direct = ds_direct_array
    result.Ds_overhang_blocked = ds_blocked_array
    if store_components:
        result.Ds = ds_array
...
def apply_overhang_material(
    basis: ComplexDaylightResult,
    visible_reflectance: float,
    specular_fraction: float,
) -> ComplexDaylightResult:
    """Recombine one traced geometry for another panel surface material.

    Ray/occlusion geometry is unchanged by material.  The saved direct and
    intercepted-sky bases make the material response algebraic, so a large
    material scan does not repeat the expensive window integration.
    """
    if basis.Ds_direct is None or basis.Ds_overhang_blocked is None:
        raise ValueError("采光结果缺少遮阳材料重组基底，请重新计算该几何。")
    result = deepcopy(basis)
    class _Surface:
        pass
    surface = _Surface()
    surface.visible_reflectance = visible_reflectance
    surface.specular_fraction = specular_fraction
    factor = _overhang_reflection_fraction(surface)
    ds = basis.Ds_direct + factor * basis.Ds_overhang_blocked
    dext = basis.Dext
    if dext is None:
        raise ValueError("采光结果缺少室外反射分量，无法重组材料响应。")
    dint = basis.rho_bar * (ds + dext) / max(1.0 - basis.rho_bar, 1e-12)
    result.Ds = ds
    result.Dint = dint
    result.DF = ds + dext + dint
    result.E_lux = result.DF / 100.0 * result.E_out
    _fill_stats(result)
    return result
...
def _fill_stats(result: ComplexDaylightResult) -> None:
    ...
    threshold = max(float(result.Ra_threshold), 1e-12)
    result.daylight_score = float(np.mean(
        np.clip(valid_df / threshold, 0.0, 1.0)
    ))
