"""
core/complex_thermal.py — Arbitrary-space single-zone thermal engine  v4.4.2
...
def compute_complex_thermal(
    ...
    result.method = (
        ...
        "实体墙内表面净面积 + 倾斜挑檐/百叶/反光板月代表工况遮阳 v4.4.0"
    )
    ...
    for wall, opening, azimuth, _factors in exterior_windows:
        ...
        for month in range(12):
            ...
            if applies and shading.has_component("horizontal_overhang"):
                overhang_block = shading.beam_shade_fraction(
                    profile, opening.height_mm,
                    depth_mm=depth, gap_mm=gap, tilt_deg=tilt,
                )
                combined_transmission *= 1.0 - overhang_block * (
                    1.0 - shading.effective_shaded_solar_residual()
                )
            if (
                applies and profile is not None
                and shading.louver_deployed(month + 1)
            ):
                aperture = shading.louver_direct_transmittance(profile)
                effectiveness = {
                    "exterior": 1.0,
                    "between": 0.72,
                    "interior": 0.38,
                }.get(str(shading.louver_position), 1.0)
                louver_transmission = 1.0 - (
                    1.0 - aperture
                ) * effectiveness * (1.0 - 0.12 * shading.louver_reflect)
                combined_transmission *= float(np.clip(
                    louver_transmission, 0.0, 1.0
                ))
            if applies and shading.has_component("light_shelf") and profile is not None:
                outside = shading.effective_shelf_outside_depth_mm()
                if outside > 0.0:
                    ratio = float(np.clip(
                        shading.light_shelf_upper_window_ratio, 0.05, 0.90
                    ))
                    lower_height = opening.height_mm * (1.0 - ratio)
                    shelf_shadow = outside * (
                        math.tan(math.radians(profile))
                        + math.tan(math.radians(shading.light_shelf_tilt_deg))
                    )
                    shelf_block = min(
                        1.0,
                        max(0.0, shelf_shadow) / max(lower_height, 1e-6),
                    ) * (1.0 - ratio)
                    shelf_transmission = 1.0 - shelf_block * (
                        1.0 - 0.10 * float(np.clip(
                            shading.light_shelf_reflect, 0.0, 1.0
                        ))
                    )
                    combined_transmission *= float(np.clip(
                        shelf_transmission, 0.0, 1.0
                    ))
            monthly_values.append(base_sc * float(np.clip(
                combined_transmission, 0.0, 1.0
            )))
        window_sc_monthly[opening.id] = monthly_values
    ...
    result.overheat_degree_months = float(np.sum(
        np.maximum(smoothed - T_COMFORT_HIGH, 0.0)
    ))
    result.underheat_degree_months = float(np.sum(
        np.maximum(T_COMFORT_LOW - smoothed, 0.0)
    ))
    result.thermal_discomfort = (
        result.overheat_degree_months + result.underheat_degree_months
    )
