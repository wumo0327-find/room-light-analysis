...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("建筑室内采光分析工具  v2.13.0  |  益阳默认气象")
    ...
    def _update_title(self):
        ...
        suf = f"  —  {fn}" if fn else ""
        self.setWindowTitle(f"建筑室内采光分析工具  v2.13.0{suf}")
    ...
    def _run_experiment(self, params: dict):
        ...
        include_glass = params["include_glass"]
        # 总进度点数：几何组合×材料 + 玻璃组（与 run_overhang_experiment 内部计数一致）
        total = (len(tilt_degs) * len(depth_mms) * len(gap_mms) * len(materials)
                 + (len(specs) if include_glass else 0))
        ...
        def _run_exp(progress_cb):
            ...
            return run_all_experiments(
                ...
                glass_specs=specs, include_glass=include_glass, progress_cb=_pcb)
    ...
    def _on_experiment_done(self, idx: int, name: str, result):
        if result is not None:
            self.experiment_panel.show_result(result, getattr(self, "_exp_params", {}))
