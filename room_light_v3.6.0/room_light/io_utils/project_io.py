"""
io_utils/project_io.py — 项目文件保存与读取  v3.6.0
...
from core.complex_models import (
    BoundaryLoop, BuildingModel, ExteriorBarrier, Point2D,
...
def building_to_dict(building: BuildingModel):
    """Serialize a v3 building without losing source/audit metadata."""
    return {
        "id": building.id,
        "name": building.name,
...
def building_from_dict(data) -> BuildingModel:
    """Parse v3 building data. Geometry is validated by the caller."""
    storeys = []
...
def save_building_project(
    ...
    """
    Save a validated v3 complex-space project.
...
def load_building_project(path: str):
    """
    Load either a v3 building project or an old rectangle project.
