...
v4.4.3 将推荐原则改为“满足成本要求后，光热环境越好越优”。年综合费用只作为进入推荐池的门槛，不再参与推荐得分；门槛内按连续采光达标度↑与热不舒适度↓的等权理想点距离选出金星方案。

v4.4.2 修正改造前基准反复成为推荐方案的问题，并将常规采光分析网格细化为150mm，让最外圈网格单元贴合实体墙内边界。

v4.4.1 提高参数化二维图、三维点云和辅助投影的材料配色饱和度与点可见度；计算结果和筛选逻辑不变。
...
- 年综合费用由优化目标改为硬门槛；成本上限内不再奖励“更便宜”。
- 推荐得分只比较采光与热不舒适度，二者归一化后等权，理想点距离最小者获推荐。
- 成本上限为0时关闭成本门槛；无成本合格方案时明确输出未达标参考方案。

### v4.4.2

- 将改造前基准明确为比较参照：存在运行费用下降的改造方案时，基准不再参与推荐。
- 最终推荐仍优先检查硬约束；无达标方案时，在节能改造池中选择残余差距最小者。
- 结果新增基准年运行费用、年运行节省额、节省率、静态回收期和推荐池说明，避免把运行节能与一次性建设费用混为一谈。
- 若没有任何改造方案降低年运行费用，维持原逻辑，允许推荐“保持现状”。

### v4.4.1

- 参数化图材料色板改为高饱和度、相互可区分的学术图表配色。
- 二维有效点和三维真实点云提高不透明度；三维辅助投影仍保持半透明层级。
- 仅改善可读性，不改变任何采光、热环境、费用、约束或帕累托计算结果。
