"""
ui/experiment_panel.py — 参数化实验结果显示面板  v4.4.3
========================================================
...
class ExperimentPanel(QWidget):
    ...
    def _update_selection_note(self):
        ...
        if self._plot_stack.currentIndex() == 1:
            ...
            note = feasibility_summary(
                ...
                p.get("minimum_criterion", "light_thermal"),
        ...
        note = (
            ...
            "金色星标先检查成本及其他硬约束，再按采光↑、热不舒适度↓等权择优，"
            "不等同于本图红圈。"
            + "\n"
            + feasibility_summary(
                self._df, p.get("constraints"),
    ...
    def _update_result_table(self):
        ...
        flagged = annotate_minimum_intervention(
            self._df, p.get("constraints"),
            criterion=p.get("minimum_criterion", "light_thermal"),
        )
