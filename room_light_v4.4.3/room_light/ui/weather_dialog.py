"""
ui/weather_dialog.py — 气象数据输入对话框  v4.4.3
三个选项卡：手动输入 / 文件导入 / 全国主要城市气象库
"""
from __future__ import annotations
from PyQt6.QtWidgets import (
    QDialog, QVBoxLayout, QHBoxLayout, QTabWidget, QWidget,
    QLabel, QTableWidget, QTableWidgetItem, QPushButton,
    QFileDialog, QHeaderView, QLineEdit, QMessageBox, QFrame, QComboBox,
)
...
from io_utils.weather_data import (
    ...
    city_weather_names, city_weather_dataset, city_weather_metadata,
)
...
class WeatherDialog(QDialog):
    ...
    def __init__(self, current: WeatherDataset | None = None, parent=None):
        ...
        tabs.addTab(self._build_city_tab(), "城市气象库")
        if (
            str(getattr(self._dataset, "location", "")) in city_weather_names()
            and "NASA POWER" in str(getattr(self._dataset, "source", ""))
        ):
    ...
    def _build_city_tab(self) -> QWidget:
        ...
        layout.setSpacing(8)
        row = QHBoxLayout()
        row.addWidget(QLabel("选择城市："))
        self._city_combo = QComboBox()
        self._city_combo.setEditable(True)
        names = city_weather_names()
        self._city_combo.addItems(names)
        current_city = str(getattr(self._dataset, "location", "") or "")
        self._city_combo.setCurrentText(
            current_city if current_city in names else "长沙"
        )
        self._city_combo.currentTextChanged.connect(self._load_city_preview)
        row.addWidget(self._city_combo, 1)
        layout.addLayout(row)

        meta = city_weather_metadata()
        note = QLabel(
            "数据源：NASA POWER（2001—2020月气候平均值）；温度=T2M，"
            "太阳辐射=ALLSKY_SFC_SW_DWN，照度按GHI×110换算。"
            "这是月均方案筛选数据，不是逐时EPW/TMY；论文定案应再用逐时气象复核。"
        )
        note.setWordWrap(True)
        note.setStyleSheet("color:#5a6175;font-size:11px;")
        note.setToolTip(str(meta.get("endpoint", "")))
        layout.addWidget(note)

        self._city_table = QTableWidget(12, 4)
        self._city_table.setHorizontalHeaderLabels(
            ["月份", "GHI (W/m²)", "室外照度 (lux)", "月均温度 (℃)"]
        )
        self._city_table.verticalHeader().setVisible(False)
        self._city_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeMode.Stretch
        )
        self._city_table.setEditTriggers(
            QTableWidget.EditTrigger.NoEditTriggers
        )
        self._city_table.setAlternatingRowColors(True)
        self._city_table.setStyleSheet(_TABLE_QSS)
        layout.addWidget(self._city_table, 1)
        self._city_dataset = None
        self._load_city_preview(self._city_combo.currentText())
        return page

    def _load_city_preview(self, city: str) -> None:
        try:
            dataset = city_weather_dataset(city)
        except Exception:
            self._city_dataset = None
            return
        self._city_dataset = dataset
        for row, values in enumerate(dataset.summary_rows()):
            for column, value in enumerate(values):
                item = QTableWidgetItem(str(value))
                item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self._city_table.setItem(row, column, item)
    ...
    def _on_ok(self):
        ...
        # Tab 0 = manual, Tab 1 = file, Tab 2 = city library
        tabs = self.findChild(QTabWidget)
        idx  = tabs.currentIndex() if tabs else 0

        if idx == 2:
            if self._city_dataset is None:
                QMessageBox.warning(self, "提示", "请选择城市气象数据。")
                return
            ds = self._city_dataset
        elif idx == 1:
            # File tab
