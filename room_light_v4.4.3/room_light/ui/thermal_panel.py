"""
ui/thermal_panel.py — 热环境分析结果面板  v4.4.3
...
class ThermalPanel(QWidget):
    def __init__(self, parent=None):
        ...
        self._comparison_result: Optional[ThermalResult] = None
    ...
    def _build_kpi_bar(self) -> QWidget:
        ...
        specs = [
            ...
            ("discomfort", "热不舒适度", "℃·月", None, False),
            ("overheat_degree", "过热累积强度", "℃·月", None, False),
            ("underheat_degree", "欠热累积强度", "℃·月", None, False),
        ]
    ...
    def _update_kpi(
        self,
        res: ThermalResult,
        comparison: Optional[ThermalResult] = None,
    ):
        vals = {
            "T_avg":     f"{res.T_in_annual_avg:.1f}",
            "T_max":     f"{float(np.max(res.T_in)):.1f}",
            "T_min":     f"{float(np.min(res.T_in)):.1f}",
            "discomfort": f"{res.thermal_discomfort:.1f}",
            "overheat_degree": f"{res.overheat_degree_months:.1f}",
            "underheat_degree": f"{res.underheat_degree_months:.1f}",
        }
        comparison_vals = None
        if comparison is not None:
            comparison_vals = {
                "T_avg": f"{comparison.T_in_annual_avg:.1f}",
                "T_max": f"{float(np.max(comparison.T_in)):.1f}",
                "T_min": f"{float(np.min(comparison.T_in)):.1f}",
                "discomfort": f"{comparison.thermal_discomfort:.1f}",
                "overheat_degree": f"{comparison.overheat_degree_months:.1f}",
                "underheat_degree": f"{comparison.underheat_degree_months:.1f}",
            }
        for key, (lbl, thresh, hi_good) in self._kpis.items():
            ...
            if thresh is not None and hi_good is not None:
                try:
                    ...
                    lbl.setStyleSheet(
                        f"color:{col};font-size:{13 if comparison else 17}px;"
                        "font-weight:700;background:transparent;")
                except Exception:
                    pass
            elif comparison_vals is not None and key in {
                "discomfort", "overheat_degree", "underheat_degree"
            }:
                before = float(vals[key])
                after = float(comparison_vals[key])
                col = C_OK if after <= before else C_HOT
                lbl.setStyleSheet(
    ...
    # ── 公共接口 ──────────────────────────────────────────────────────────
    def update(self, result: ThermalResult, room: RoomModel | SpaceModel):
        self._result = result
        self._room   = room
    ...
    def set_comparison(
        ...
        optimal_result: ThermalResult,
        optimal_room: RoomModel | SpaceModel,
    ):
        """Overlay the recommended scheme on the current-room thermal result."""
    ...
    def _draw_placeholder(self):
        ...
        ax.text(0.5,0.54,"在建筑视图选择房间后，点击「▶ 分析选中房间」生成热环境分析",
    ...
    def _render_line(self, ax):
        ...
        ax.set_title(
            f"逐月自然室温   当前年均 {res.T_in_annual_avg:.1f}℃"
            + (
                f" → 最优 {comparison.T_in_annual_avg:.1f}℃   "
        ...
        ax.grid(True, which="major", color="#e2e8f0", lw=0.55, alpha=0.95)
        ax.grid(True, which="minor", color="#f1f5f9", lw=0.35, alpha=0.95)
    ...
    def _render_heatflow(self, ax):
        ...
        ax.yaxis.set_minor_locator(mticker.AutoMinorLocator(2))
        ax.xaxis.set_minor_locator(mticker.MultipleLocator(0.5))
        ax.grid(True, which="major", axis="y", color="#e2e8f0",
                lw=0.55, alpha=0.95)
        ax.grid(True, which="minor", axis="y", color="#f1f5f9",
                lw=0.35, alpha=0.95)
        for sp in ax.spines.values():
            sp.set_color(C_BORDER); sp.set_linewidth(0.6)
        ax.tick_params(colors=C_TEXTSEC, labelsize=8, length=3)
