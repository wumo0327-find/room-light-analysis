"""
ui/experiment_sidebar.py — 参数化实验专用侧边栏  v4.4.3
================================================================
...
class ExperimentSidebar(QWidget):
    ...
    def _build_decision_section(self):
        ...
        priority_note = QLabel(
            "推荐规则：成本只作为门槛；满足成本及其他硬约束后，按采光更高、"
            "热不舒适度更低进行等权光热择优，不再把最低造价作为推荐目标。"
        )
        priority_note.setWordWrap(True)
        priority_note.setStyleSheet(
            "color:#2563eb;font-size:10px;background:transparent;"
        )
        sec.add_widget(priority_note)
        self._main.addWidget(sec)
    ...
    def _on_run(self):
        ...
        params = {
            ...
            "minimum_criterion": "light_thermal",
