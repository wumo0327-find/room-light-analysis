...
from core.models import (
    RoomModel, ShadingDevice, DEFAULT_SUPPORT_COST_PER_M,
    DEFAULT_INSTALL_COST_PER_WINDOW, canonical_material_name,
)
...
def build_solution_room(base_room: RoomModel, solution) -> RoomModel:
    ...
    room.shading = ShadingDevice(
        ...
        visible_reflectance=float(spec.get("visible_reflectance", 0.32)),
        solar_reflectance=float(spec.get("solar_reflectance", 0.32)),
        thermal_emissivity=float(spec.get("thermal_emissivity", 0.90)),
        specular_fraction=float(spec.get("specular_fraction", 0.03)),
    )
    return room


def _daylight_metrics(dl) -> dict:
    """从 DaylightResult 提取采光指标。"""
    return {
        "Ra": dl.Ra,
        "daylight_score": dl.daylight_score,
        "U0": dl.U0,
        "DF_avg": dl.DF_avg,
        "E_avg": dl.E_avg,
    }
...
def run_overhang_experiment(
    ...
    v4.2.2 起每个材料都独立计算采光与热环境。可见光反射率参与挑檐下表面
    一次反射补光估算，不再把同一几何的采光结果复制给全部材料。
    ...
    for m in (materials or []):
        spec = get_material(m)
        if spec is not None:
            canonical_name = canonical_material_name(m)
            default_price = float(spec.get(
                "installed_cost_per_m2", spec.get("cost", 230.0)))
            unit_price = float(material_unit_costs.get(
                m, material_unit_costs.get(canonical_name, default_price)))
            mat_specs.append((canonical_name, dict(spec), spec["color"], unit_price))
    if not mat_specs:
        mat_specs = [("默认(k_diff=0.30)", {
            "k_diff": 0.30, "visible_reflectance": 0.32,
            "solar_reflectance": 0.32, "thermal_emissivity": 0.90,
    ...
    for (tilt, L_mm, gap_mm) in geom_combos:
        ...
        for (mat_name, material_spec, color, unit_cost) in mat_specs:
            if progress_cb:
                progress_cb(done, total)
            done += 1
            room.shading.diffuse_residual = float(material_spec.get("k_diff", 0.30))
            room.shading.visible_reflectance = float(material_spec.get("visible_reflectance", 0.32))
            room.shading.solar_reflectance = float(material_spec.get("solar_reflectance", 0.32))
            room.shading.thermal_emissivity = float(material_spec.get("thermal_emissivity", 0.90))
            room.shading.specular_fraction = float(material_spec.get("specular_fraction", 0.03))
            dl = compute_daylight(room, E_out=weather.annual_avg,
                                  ndiv=ndiv, ra_threshold=ra_threshold)
            ...
            rec.update({
                ...
                "gap_mm": float(gap_mm),
                "material": mat_name,
                "visible_reflectance": room.shading.visible_reflectance,
                "solar_reflectance": room.shading.solar_reflectance,
                "thermal_emissivity": room.shading.thermal_emissivity,
                "specular_fraction": room.shading.specular_fraction,
...
def run_all_experiments(
    ...
    if not df_o.empty:
        parts.append(df_o)
    cols = ["group", "param_label", "param_value", "Ra", "daylight_score", "U0", "comfort_ratio",
...
_PARETO_ATOL = {
    ...
    "U0": 1e-4,
    "cost": 0.5,
    "thermal_discomfort": 0.01,
    "overheat_degree_months": 0.01,
    "underheat_degree_months": 0.01,
    "comfort_ratio": 1e-4,
    "comfort_months": 1e-6,
}
...
def pareto_front(
    ...
    front = pareto_front_nd(
        df,
        [x, y],
        maximize=[maximize_x, maximize_y],
        u0_col=u0_col,
        u0_min=u0_min,
    )
    if front.empty:
        return front
    # Only sorting is used to draw the boundary. Pairwise dominance above is
    # the source of truth and deliberately preserves all exactly equivalent
    # schemes/materials instead of silently keeping whichever row came first.
...
def pareto_front_nd(
    ...
    if u0_col in d.columns and u0_min is not None:
        d = d[d[u0_col] >= u0_min]
    if d.empty or not cols or not set(cols).issubset(d.columns):
        return d
    finite = np.all(np.isfinite(d[cols].to_numpy(dtype=float)), axis=1)
    d = d.loc[finite]
    if d.empty:
        return d
    if maximize is None:
    ...
    for j, mx in enumerate(maximize):
        if not mx:
            vals[:, j] = -vals[:, j]
    # Engineering outputs are displayed and interpreted at finite resolution.
    # These small absolute tolerances prevent floating-point noise from making
    # a physically indistinguishable scheme look Pareto-superior.
    tolerances = np.asarray([
        float(_PARETO_ATOL.get(column, 1e-9)) for column in cols
    ])
    n = len(vals)
    ...
    for i in range(n):
        ...
        for j in range(n):
            if i == j:
                continue
            if (
                np.all(vals[j] >= vals[i] - tolerances)
                and np.any(vals[j] > vals[i] + tolerances)
            ):
                dominated[i] = True
                break
...
# 普通样本点按当前候选总量自动缩放。Matplotlib 的 ``s`` 是面积(points²)，
# 所以数千样本时必须降到约1 points²，才不会把相邻方案涂成色块。
def adaptive_marker_area(sample_count: int, dimensions: int = 2) -> float:
    ...
    if count <= 120:
        ...
        area = 1.0
    return area * (0.82 if dimensions == 3 else 1.0)
...
def _light_objective_col(df: pd.DataFrame) -> str:
    """Prefer the continuous daylight objective, with Ra fallback for old data."""
    if "daylight_score" in df.columns:
        values = pd.to_numeric(df["daylight_score"], errors="coerce")
        if values.notna().any():
            return "daylight_score"
    return "Ra"
...
def global_pareto_front(
    ...
    目标为连续采光达标度Cd↑（旧数据回退Ra↑）、热轴和cost↓；基准不参与。
    """
    d = _shading_candidates(df, u0_col=u0_col, u0_min=u0_min)
    light_col = _light_objective_col(d)
    needed = {light_col, thermal_col, "cost"}
    if d.empty or not needed.issubset(d.columns):
        return d.iloc[0:0]
    return pareto_front_nd(
        d, [light_col, thermal_col, "cost"],
...
def balanced_compromise(
    ...
    d = d.sort_values(
        ["_ideal_distance", light_col, thermal_col, "cost"],
...
def _decision_or_balanced(
    df: pd.DataFrame, thermal_col: str, maximize_thermal: bool,
    u0_col: str, u0_min: float,
) -> pd.DataFrame:
    """Use the preregistered minimum-intervention decision when available."""
    if "decision_recommended" in df.columns:
        chosen = df[df["decision_recommended"].fillna(False).astype(bool)]
        if not chosen.empty:
...
def annotate_global_selection(
    ...
    """Add stable IDs, 2D/3D Pareto flags and a traceable compromise rank."""
    out = df.copy()
    candidates = _shading_candidates(
        out, u0_col=u0_col, u0_min=u0_min
    )
    out["solution_id"] = ""
    for number, index in enumerate(candidates.index, start=1):
        out.at[index, "solution_id"] = f"S{number:04d}"
    references = out.index.difference(candidates.index)
    for number, index in enumerate(references, start=1):
        out.at[index, "solution_id"] = f"B{number:03d}"

    pf = global_pareto_front(
        out, thermal_col=thermal_col, maximize_thermal=maximize_thermal,
        u0_col=u0_col, u0_min=u0_min)
    rec = _decision_or_balanced(
        out, thermal_col=thermal_col, maximize_thermal=maximize_thermal,
        u0_col=u0_col, u0_min=u0_min)
    out["global_pareto"] = out.index.isin(pf.index)
    out["balanced_recommended"] = out.index.isin(rec.index)
    light_col = _light_objective_col(out)
    out["light_objective"] = light_col
    if {light_col, thermal_col, "cost"}.issubset(out.columns):
        out["pareto_light_thermal_2d"] = out.index.isin(pareto_front(
            candidates, light_col, thermal_col, True, maximize_thermal,
            u0_col=u0_col, u0_min=u0_min,
        ).index)
        out["pareto_light_cost_2d"] = out.index.isin(pareto_front(
            candidates, light_col, "cost", True, False,
            u0_col=u0_col, u0_min=u0_min,
        ).index)
        out["pareto_thermal_cost_2d"] = out.index.isin(pareto_front(
            candidates, thermal_col, "cost", maximize_thermal, False,
            u0_col=u0_col, u0_min=u0_min,
        ).index)
    out["ideal_distance"] = np.nan
    out["recommendation_rank"] = np.nan
    if not candidates.empty and {light_col, thermal_col}.issubset(candidates.columns):
        # 成本是进入推荐池的门槛，不再计入推荐距离；排名只反映光热表现。
        distances = np.sqrt((
            (1.0 - _benefit(candidates[light_col], True)) ** 2
            + (1.0 - _benefit(candidates[thermal_col], maximize_thermal)) ** 2
        ) / 2.0)
        out.loc[candidates.index, "ideal_distance"] = distances
        if "light_thermal_rank" in out.columns:
            ranked = pd.to_numeric(
                out.loc[candidates.index, "light_thermal_rank"], errors="coerce"
            ).dropna().sort_values(kind="mergesort")
            order = ranked.index.append(
                distances.drop(index=ranked.index, errors="ignore")
                .sort_values(kind="mergesort").index
            )
        else:
            order = distances.sort_values(kind="mergesort").index
        for rank, index in enumerate(order, start=1):
            out.at[index, "recommendation_rank"] = rank
    return out
...
def global_selection_summary(
    ...
    light_col = _light_objective_col(df)
    light_label = "连续采光达标度Cd" if light_col == "daylight_score" else "采光达标面积比Ra"
    ...
    displayed_thermal_tolerance = _PARETO_ATOL.get(thermal_col, 1e-9)
    rule = (
        "图示说明：每张2D图及3D对应投影面的红圈/虚线，按该面两个指标单独"
        "判断二维帕累托；3D空中点云红圈才表示三个指标共同判断的全局帕累托。"
        "全局筛选标准：全部材料×倾角θ×板长L×间隙h进入同一候选池；"
        f"{u0_rule}若另一方案同时满足 {light_label}不低、{thermal_rule}、年综合费用不高，"
        "且至少一项严格更好，则本方案被支配并淘汰。原始模型基准仅作改造前参照。"
        f"数值等价容差为{light_label}±0.0001、{thermal_label}±{displayed_thermal_tolerance:g}、"
        "年综合费用±0.5元/年。"
    )
    ...
    if "material" in candidates.columns:
        ...
        geometry_eps = 1e-9
        ra_tol = _PARETO_ATOL.get(light_col, 1e-4)
        thermal_tol = _PARETO_ATOL.get(thermal_col, 1e-9)
        cost_tol = _PARETO_ATOL["cost"]

        def dominates(other: pd.Series, row: pd.Series) -> bool:
            ra_ok = float(other[light_col]) >= float(row[light_col]) - ra_tol
            if maximize_thermal:
                thermal_ok = float(other[thermal_col]) >= float(row[thermal_col]) - thermal_tol
                thermal_strict = float(other[thermal_col]) > float(row[thermal_col]) + thermal_tol
            else:
                thermal_ok = float(other[thermal_col]) <= float(row[thermal_col]) + thermal_tol
                thermal_strict = float(other[thermal_col]) < float(row[thermal_col]) - thermal_tol
            cost_ok = float(other["cost"]) <= float(row["cost"]) + cost_tol
            strict = (
                float(other[light_col]) > float(row[light_col]) + ra_tol
                or thermal_strict
                or float(other["cost"]) < float(row["cost"]) - cost_tol
            )
        ...
        for material in material_order:
            ...
            if dominated_same_geometry == len(material_rows) and len(material_rows):
                ...
                reasons.append(
                    ...
                    f"均被{names}支配（{light_label}不低，{thermal_rule}且年综合费用不高，至少一项更优）"
                )
            else:
                reasons.append(
    ...
    if not rec.empty:
        ...
        recommendation = (
            f"决策推荐（金星）：{rec_name}。成本只作为门槛；在满足成本及其他"
            "硬约束的方案中，按采光↑与热不舒适度↓等权择优。"
        )
    else:
        recommendation = "决策推荐：没有可参与计算的候选方案。"
...
def _axis_ticks(series: pd.Series) -> np.ndarray:
    """2D和3D共用的主刻度；相同指标必须得到完全相同的分格。"""
    ...
    nice = next(
        step for step in (1.0, 2.0, 2.5, 5.0, 10.0)
        if step >= normalized - 1e-12
    )
    step = nice * magnitude
    tolerance = max(span, 1.0) * 1e-10
    start = math.ceil((lo - tolerance) / step) * step
    end = math.floor((hi + tolerance) / step) * step
    count = max(0, int(round((end - start) / step)) + 1)
    ticks = start + np.arange(count, dtype=float) * step
    # Remove binary tails such as 0.35000000000000003 from tick labels.
...
def plot_experiments(
    ...
    表示跨全部材料/几何的二维帕累托点和边界；金星是成本门槛内的光热等权推荐。
    原始工程/玻璃只作参照。
    """
    ...
    legend_seen = {}
    marker_area = adaptive_marker_area(len(df), dimensions=2)
    ...
    for grp, key, mk, sub in _color_subgroups(df):
        ...
        if len(comp):
            ...
            ax.scatter(df.loc[comp, x], df.loc[comp, y], s=marker_area,
                       marker=mk, c=col, alpha=0.86, edgecolors="none",
                       label=lab, zorder=3)
        if len(ncomp):
            ax.scatter(df.loc[ncomp, x], df.loc[ncomp, y], s=max(0.8, marker_area * 0.75),
                       marker="x", c=_C_NONCOMP, alpha=0.45, zorder=2)

    shading = _shading_candidates(df, u0_col=u0_col, u0_min=u0_min)
    recommended = _decision_or_balanced(
    ...
    if show_pareto and not shading.empty:
        ...
        if not pf2.empty:
            ...
            pareto_area = 12.0 if len(pf2) <= 80 else 7.0 if len(pf2) <= 300 else 4.0
            pareto_linewidth = 1.1 if len(pf2) <= 300 else 0.65
            ax.plot(pf2[x], pf2[y], "--", color=_C_PARETO, lw=1.25,
                    alpha=0.72, zorder=4)
            ax.scatter(
                pf2[x], pf2[y], s=max(pareto_area, marker_area * 3.0),
                marker="o", facecolors="none", edgecolors=_C_PARETO,
                linewidths=pareto_linewidth, zorder=5)

    if not recommended.empty:
        r = recommended.iloc[0]
        ax.scatter([r[x]], [r[y]], s=58.0, marker="*",
                   c=_C_RECOMMEND, edgecolors="#92400e", linewidths=0.8, zorder=7)
        if "param_label" in recommended.columns:
            ax.annotate(
                f"决策推荐：{r['param_label']}",
    ...
    ax.set_xticks(_axis_ticks(df[x]))
    ax.set_yticks(_axis_ticks(df[y]))
...
# ── 3D 全局帕累托点云 ───────────────────────────────────────────────────────
_3D_VIEW_ELEV, _3D_VIEW_AZIM = 18.0, 34.0
# Matplotlib 的默认 focal_length=1 接近“长焦”效果。较短焦距能明显加强
# 近大远小，但过短会令坐标面畸变；0.32 在论文图可读性和透视感之间较稳妥。
_3D_FOCAL_LENGTH = 0.32
...
def build_pareto3d_figure(
    ...
    采用原始指标值构建透视3D图，返回(fig, ax)：
      X=连续采光达标度Cd（旧CSV回退Ra）、Y=年综合费用、Z=所选热指标。
      热轴（当越小越好时）与费用轴反向显示，使低热不舒适度、低年综合费用位于外侧；
      三个投影面固定在各目标的较差边界并交于空间参考点O。O只表示三面交点，
      不表示数值必须为(0,0,0)。光×热投影位于竖直侧面，三面坐标方向与对应
      的三张2D图完全一致。
    ...
    marker_area = adaptive_marker_area(len(d), dimensions=3)

    fig = Figure(figsize=(7.2, 6.4), facecolor=_C_BG)
    ax = fig.add_subplot(111, projection="3d")
    ax.set_facecolor(_C_BG)

    light_min, light_max = _axis_limits(d[light_col])
    cost_min, cost_max = _axis_limits(d["cost"])
    thermal_min, thermal_max = _axis_limits(d[thermal_col])
    # O位于三个目标的较差参考角：低采光优度、高热不舒适度（若越小越好）、高年综合费用。
    # 若用户选择“舒适月数占比”等越大越好的热轴，则热投影面改放在其低值边界。
    light_plane = light_min
    cost_plane = cost_max
    thermal_plane = thermal_min if maximize_thermal else thermal_max
    ...
    for grp, key, mk, sub in _color_subgroups(comp_df):
        ...
        if len(non_pf):
            ax.scatter(non_pf[light_col], non_pf["cost"], non_pf[thermal_col],
                       s=marker_area, marker=mk, c=col, alpha=0.62,
                       edgecolors="none", depthshade=False)
        if len(pf_pts):
            ax.scatter(pf_pts[light_col], pf_pts["cost"], pf_pts[thermal_col],
                       s=max(1.2, marker_area), marker=mk, c=col, alpha=0.96,
                       edgecolors="none", depthshade=False)
    ncomp = d[~compliant]
    if len(ncomp):
        ax.scatter(ncomp[light_col], ncomp["cost"], ncomp[thermal_col],
                   s=max(0.7, marker_area * 0.7), marker="x",
                   c=_C_NONCOMP, alpha=0.38)

    # 三面投影只作为可选辅助层。候选较多时默认关闭，避免真实点云和三份重复
    # 投影叠成无法辨认的“六倍点数”。开启后仍严格落在三个网格面上。
    if show_projections:
        for grp, key, mk, sub in _color_subgroups(comp_df):
            col = sub["_plot_color"].iloc[0] if has_color else _C_ACCENT
            # 光×热：竖直 XZ 侧面（Y=最高年综合费用）。
            ax.scatter(sub[light_col], np.full(len(sub), cost_plane), sub[thermal_col],
                       s=max(0.6, marker_area * 0.65), c=col, marker=mk,
                       alpha=0.30, edgecolors="none", depthshade=False)
            # 光×成本：水平 XY 面（Z=最差热指标）。
            ax.scatter(sub[light_col], sub["cost"], np.full(len(sub), thermal_plane),
                       s=max(0.6, marker_area * 0.65), c=col, marker=mk,
                       alpha=0.30, edgecolors="none", depthshade=False)
            # 热×成本：竖直 YZ 面（X=最低采光优度）。
            ax.scatter(np.full(len(sub), light_plane), sub["cost"], sub[thermal_col],
                       s=max(0.6, marker_area * 0.65), c=col, marker=mk,
                       alpha=0.30, edgecolors="none", depthshade=False)

    shading = _shading_candidates(d, u0_col=u0_col, u0_min=u0_min)
    has_projection_front = False
    if show_pareto and show_projections and not shading.empty:
        # 三个二维前沿只贴在对应坐标面上。
        fronts = [
            (pareto_front(shading, light_col, thermal_col, True, maximize_thermal,
                          u0_col, u0_min), light_col, "light_thermal"),
            (pareto_front(shading, light_col, "cost", True, False,
                          u0_col, u0_min), light_col, "light_cost"),
            (pareto_front(shading, thermal_col, "cost", maximize_thermal, False,
                          u0_col, u0_min), "cost", "thermal_cost"),
        ]
        for front, sort_col, plane in fronts:
            if front.empty:
                continue
            has_projection_front = True
            front = d.loc[front.index].sort_values(sort_col)
            projection_ring_area = (
                10.0 if len(front) <= 80
                else 6.0 if len(front) <= 300
                else 3.5
            )
            ring2d_kw = dict(
                s=max(projection_ring_area, marker_area * 3.0), marker="o", facecolors="none",
                edgecolors=_C_PARETO, linewidths=0.8,
                alpha=0.72, depthshade=False)
            if plane == "light_thermal":
                ax.plot(front[light_col], np.full(len(front), cost_plane),
                        front[thermal_col], "--",
                        color=_C_PARETO, lw=1.0, alpha=0.65)
                ax.scatter(
                    front[light_col], np.full(len(front), cost_plane),
                    front[thermal_col],
                    **ring2d_kw)
            elif plane == "light_cost":
                ax.plot(front[light_col], front["cost"],
                        np.full(len(front), thermal_plane), "--",
                        color=_C_PARETO, lw=1.0, alpha=0.65)
                ax.scatter(
                    front[light_col], front["cost"],
                    np.full(len(front), thermal_plane),
                    **ring2d_kw)
            else:
                ax.plot(np.full(len(front), light_plane), front["cost"],
                        front[thermal_col], "--",
                        color=_C_PARETO, lw=1.0, alpha=0.65)
                ax.scatter(
                    np.full(len(front), light_plane), front["cost"],
                    front[thermal_col],
                    **ring2d_kw)

    # 三目标全局帕累托始终只圈空中的真实点，与投影层开关无关。
    if show_pareto and not pf_global.empty:
        p = d.loc[pf_global.index]
        ring_area = 14.0 if len(p) <= 80 else 8.0 if len(p) <= 300 else 4.5
        ring_width = 1.15 if len(p) <= 300 else 0.65
        ring_kw = dict(s=max(ring_area, marker_area * 4.0), marker="o", facecolors="none",
                       edgecolors=_C_PARETO, linewidths=ring_width,
                       depthshade=False)
        ax.scatter(p[light_col], p["cost"], p[thermal_col], **ring_kw)

    # 均衡推荐始终显示，独立于帕累托红圈/虚线开关。
    if not recommended.empty:
        r = d.loc[recommended.index[0]]
        star_kw = dict(s=62, marker="*", c=_C_RECOMMEND,
                       edgecolors="#92400e", linewidths=0.8, depthshade=False)
        ax.scatter([r[light_col]], [r["cost"]], [r[thermal_col]], **star_kw)
        if show_projections:
            proj_star = dict(star_kw); proj_star.update(s=58, alpha=0.68)
            ax.scatter([r[light_col]], [cost_plane], [r[thermal_col]], **proj_star)
            ax.scatter([r[light_col]], [r["cost"]], [thermal_plane], **proj_star)
            ax.scatter([light_plane], [r["cost"]], [r[thermal_col]], **proj_star)

    ax.set_xlim(light_min, light_max)
    ax.set_ylim(cost_max, cost_min)
    ax.set_zlim(
        (thermal_min, thermal_max)
        if maximize_thermal else (thermal_max, thermal_min)
    )
    ax.set_xticks(_axis_ticks(d[light_col]))
    ax.set_yticks(_axis_ticks(d["cost"]))
    ax.set_zticks(_axis_ticks(d[thermal_col]))
    thermal_arrow = "↑" if maximize_thermal else "↓"
    ax.set_xlabel(f"{light_label} ↑", color=_C_TEXT, fontsize=9, labelpad=8)
    ax.set_ylabel("年综合费用（元/年） ↓", color=_C_TEXT, fontsize=9, labelpad=8)
    ax.set_zlabel(f"{thermal_label} {thermal_arrow}", color=_C_TEXT, fontsize=9, labelpad=8)
    ax.tick_params(colors=_C_SEC, labelsize=7)
    ax.set_title(
        f"遮阳方案三目标全局权衡｜候选{len(shading)}｜全局前沿{len(pf_global)}\n"
        "固定广角透视｜光×热位于竖直侧面；低热不舒适度、低年综合费用向外",
        fontsize=9.2, color=_C_TEXT, pad=4)

    # 三个面使用不同透明度建立空间层次；重点强调承载光×热投影的 XZ 侧面。
    ax.xaxis.set_pane_color((0.96, 0.98, 1.00, 0.16))  # YZ：热×成本
    ax.yaxis.set_pane_color((0.93, 0.97, 1.00, 0.34))  # XZ：光×热
    ax.zaxis.set_pane_color((0.98, 0.99, 1.00, 0.12))  # XY：光×成本
    ax.grid(True)
    # mplot3d 不读取普通 ax.grid(color=...) 的全部样式，需给三根轴分别设置。
    for axis in (ax.xaxis, ax.yaxis, ax.zaxis):
        axis._axinfo["grid"].update({
            "color": (0.57, 0.63, 0.72, 0.30),
            "linewidth": 0.55,
            "linestyle": "-",
        })
        axis._axinfo["axisline"].update({
            "color": (0.27, 0.33, 0.42, 0.72),
            "linewidth": 0.75,
        })
    ax.set_proj_type("persp", focal_length=_3D_FOCAL_LENGTH)
    ax.set_box_aspect((1.35, 1.00, 0.92), zoom=0.90)
    ...
    material_rows = _shading_candidates(
        comp_df, u0_col=u0_col, u0_min=u0_min
    )
    if "material" in material_rows.columns:
        for material, rows in material_rows.groupby("material", sort=False):
            color = (
                rows["_plot_color"].iloc[0]
                if has_color else _C_ACCENT
            )
    ...
    if reference.any():
        handles.append(Line2D(
            ...
            label="改造前基准"))
    ax.legend(handles=handles, fontsize=7.0, loc="upper left",
