"""
core/space_geometry.py — Deterministic polygon geometry and validation  v4.4.3
...
def space_floor_area_mm2(space: SpaceModel) -> float:
    outer_points = space.analysis_outer_points()
    if not outer_points:
        return 0.0
    area = ring_area_mm2(outer_points)
    area -= sum(
        ring_area_mm2(points)
        for points in space.analysis_hole_points()
    )
    return max(0.0, area)
...
def point_in_space(
    ...
    outer_points = space.analysis_outer_points()
    if not outer_points or not point_in_ring(
        point, outer_points, include_boundary
    ):
        return False
    return not any(
        point_in_ring(point, points, include_boundary=False)
        for points in space.analysis_hole_points()
    )
...
def validate_space(
    ...
    # v4 separates authoring/host axes from the net usable-floor boundary.
    # This keeps corner windows attached to their original wall axes while
    # calculation points follow the physical inner wall faces.
    for floor_loop in space.analysis_floor_loops:
        points = floor_loop.points
        if floor_loop.kind not in LOOP_KINDS:
            issues.append(GeometryIssue(
                "unknown_analysis_floor_kind",
                f"未知净空间边界类型：{floor_loop.kind}。",
                object_id=floor_loop.id,
            ))
            continue
        if len(points) < 3:
            issues.append(GeometryIssue(
                "analysis_floor_too_few_points",
                "净空间边界至少需要三个点。",
                object_id=floor_loop.id,
            ))
            continue
        if ring_area_mm2(points) <= EPSILON_MM:
            issues.append(GeometryIssue(
                "analysis_floor_zero_area",
                "净空间边界面积为0。",
                object_id=floor_loop.id,
            ))
        for first_index, second_index in _non_adjacent_self_intersections(points):
            issues.append(GeometryIssue(
                "analysis_floor_self_intersection",
                f"净空间边界第{first_index + 1}段与第{second_index + 1}段相交。",
                object_id=floor_loop.id,
            ))
