"""
ui/experiment_sidebar.py — 参数化实验专用侧边栏  v3.1.0
================================================================
...
class ExperimentSidebar(QWidget):
    ...
    def _build_material_section(self):
        ...
        note = QLabel("只计算勾选材料；右侧为板材加工/表面处理综合单价(元/㎡)，"
                      "请按项目所在地信息价或厂家报价修改。所有材料共同参与全局筛选。")
        note.setWordWrap(True)
        note.setStyleSheet("color:#9aa0b0;font-size:10px;background:transparent;")
        sec.add_widget(note)
        for cat, mats in MATERIAL_LIBRARY.items():
            ...
            for name, spec in mats.items():
                ...
                cb.setToolTip(f"k_diff={spec['k_diff']:.2f}；材料只影响热环境，不改变采光几何")
                price = QDoubleSpinBox()
                price.setRange(0.0, 100000.0)
                price.setDecimals(0)
                price.setSingleStep(50.0)
                price.setSuffix(" 元/㎡")
                price.setValue(float(spec.get("installed_cost_per_m2", 750.0)))
        ...
        self._support_cost.setValue(180.0)
    ...
    def set_complex_project_context(
        ...
        if space is None:
            ...
            wall_openings = exterior_windows(space)
            has_windows = bool(wall_openings)
            ...
            detail = (
                f"空间 {space.name}；面积 "
                f"{space_floor_area_mm2(space) / 1_000_000:.2f} ㎡；"
                f"层高 {space.height_mm / 1000:.2f} m；"
                f"涉及外墙 {len(shaded_walls)}面、外窗 {len(windows)}扇；"
                f"玻璃τ={tau_text}"
            )
            self._project_has_shading = space_has_horizontal_shading(space)
            if (
        ...
        self._project_context.setText(
            f"{name}\n{detail}；气象={wx or '默认'}\n"
            "本次每组θ/L/h/材料统一应用到当前空间全部外窗；"
            f"原模型保留为改造前基准，L=0只计算一次。{vertical_note}"
        )
        self._btn_run.setEnabled(has_windows)
        self._btn_run.setToolTip(
            "" if has_windows else "当前复杂空间没有外窗，无法运行遮阳实验。"
        )
