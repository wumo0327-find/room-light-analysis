...
> [THEORY.md](THEORY.md)。v3.1.0 已将参数化遮阳实验迁移到任意多边形活动空间。
...
- 参数化遮阳实验不再调用旧矩形 `RoomModel`，直接克隆当前活动
  `SpaceModel`，使用真实多边形、外墙朝向、外窗位置/尺寸、逐窗玻璃和当前气候。
- 当前阶段把同一组水平遮阳 `θ×L×h×材料` 统一应用到活动空间全部外窗；原工程
  状态作为改造前基准，`L=0`只计算一次，不随角度、间隙或材料重复。
...
- 均衡推荐方案会重新运行完整复杂空间采光和热环境，可在结果页查看，并随四图、
  CSV和综合Excel一起导出；导出的算例增加空间面积、外墙/外窗数和应用范围。
