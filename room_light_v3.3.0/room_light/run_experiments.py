"""
run_experiments.py — 参数化实验命令行入口  v2.12.0
==================================================
不启动 GUI，批量运行「玻璃对照 + 遮阳θ×L扫描」两组实验，
导出结果表(CSV)与散点气泡图(PNG)。

v2.12.0：遮阳组不再用抽象特征角β反算深度，改为直接扫描"倾斜角θ×板长L"网格
（θ=90°为水平，>90°上扬，<90°下垂；L=板长mm，沿板自身方向）。
...
def main():
    ...
    ap.add_argument("--tilt-min", type=float, default=60.0, help="挑檐倾斜角下限(°，90=水平)")
    ap.add_argument("--tilt-max", type=float, default=120.0, help="挑檐倾斜角上限(°，90=水平)")
    ap.add_argument("--tilt-step", type=float, default=10.0, help="挑檐倾斜角步长(°)")
    ap.add_argument("--depth-min", type=float, default=300.0, help="挑檐板长下限(mm)")
    ap.add_argument("--depth-max", type=float, default=1500.0, help="挑檐板长上限(mm)")
    ap.add_argument("--depth-step", type=float, default=300.0, help="挑檐板长步长(mm)")
    ...
    n_tilt = int(round((args.tilt_max - args.tilt_min) / args.tilt_step)) + 1
    tilt_degs = [round(args.tilt_min + i * args.tilt_step, 3) for i in range(n_tilt)]
    n_depth = int(round((args.depth_max - args.depth_min) / args.depth_step)) + 1
    depth_mms = [round(args.depth_min + i * args.depth_step, 3) for i in range(n_depth)]
    ...
    df = ex.run_all_experiments(
        ...
        tilt_degs=tilt_degs, depth_mms=depth_mms, progress_cb=prog)
    print()
