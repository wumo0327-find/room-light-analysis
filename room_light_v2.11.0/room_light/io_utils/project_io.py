"""
io_utils/project_io.py — 项目文件保存与读取  v2.10.2
...
def load_project(path: str) -> Tuple[RoomModel, Optional[WeatherDataset], str]:
    ...
    try:
        ...
        overhang_overrides = {
            str(k): {"depth_mm": float(v.get("depth_mm", 0.0)),
                     "gap_mm":   float(v.get("gap_mm",   0.0))}
            for k, v in raw_over.items() if isinstance(v, dict)
        }
