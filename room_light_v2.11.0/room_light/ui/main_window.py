...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("建筑室内采光分析工具  v2.11.0  |  益阳默认气象")
        self.resize(1200, 760)   # v2.11.0: 默认非全屏窗口调小（原1480×900）
    ...
    def _switch_view(self, idx: int):
        ...
        if idx == 3:   # 切到"参数化实验"时，H(mm) 与当前窗户窗高同步
            self.experiment_panel.sync_H_from_room(self.room)
    ...
    def _update_title(self):
        ...
        suf = f"  —  {fn}" if fn else ""
        self.setWindowTitle(f"建筑室内采光分析工具  v2.11.0{suf}")
