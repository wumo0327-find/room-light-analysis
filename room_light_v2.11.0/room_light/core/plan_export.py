"""
core/plan_export.py — 房间平面图绘制与测点叠加导出  v2.10.2（v2.8.0 新增，v2.9.1 天正风格标注+遮阳可视化，v2.10.0 支持逐窗/逐位置遮阳覆盖+两端外墙翼板，v2.10.2 修正端墙柱宽为固定值非整段端墙宽度）
======================================================================
...
def draw_shading_plan(ax, room: RoomModel, fontsize=8, label_beyond_mm=0.0):
    ...
    if has_overhang:
        ...
        for win in wins:
            D, _gap = sh.get_overhang_for(win.id)   # v2.10: 逐窗覆盖
            if D <= 0.0:
                continue
...
def draw_shading_elevation(ax, room: RoomModel, wall: str, fontsize=8):
    ...
    if has_overhang:
        for win in wins:
            D, _gap = sh.get_overhang_for(win.id)
            if D <= 0.0:
                continue
            overhang_drawn = True
            ax.add_patch(patches.Rectangle(
