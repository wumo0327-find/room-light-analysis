"""
io_utils/project_io.py — 项目文件保存与读取  v4.4.1
    ...
    data = {
        ...
        "room": {
            ...
            "shading": {
                ...
                "target_window_ids":     s.target_window_ids,
                ...
                "louver_position":       s.louver_position,
                "louver_control_mode":   s.louver_control_mode,
                "light_shelf_depth_mm":  s.light_shelf_depth_mm,
                "light_shelf_reflect":   s.light_shelf_reflect,
                "light_shelf_inside_depth_mm": s.light_shelf_inside_depth_mm,
                "light_shelf_outside_depth_mm": s.light_shelf_outside_depth_mm,
                "light_shelf_tilt_deg":  s.light_shelf_tilt_deg,
                "light_shelf_height_mm": s.light_shelf_height_mm,
                "light_shelf_upper_window_ratio": s.light_shelf_upper_window_ratio,
            },
            "location": {
                "latitude":        l.latitude,
        ...
        "weather": {
            ...
            "data_period":  weather.data_period,
...
def load_project(path: str) -> Tuple[RoomModel, Optional[WeatherDataset], str]:
    ...
    try:
        ...
        room.shading = ShadingDevice(
            ...
            components         = [str(v) for v in sd.get("components", [])],
            target_window_ids  = [str(v) for v in sd.get("target_window_ids", [])],
            ...
            louver_position    = str(sd.get("louver_position", "exterior")),
            louver_control_mode= str(sd.get("louver_control_mode", "fixed")),
            light_shelf_depth_mm  = float(sd.get("light_shelf_depth_mm",  0.0)),
            light_shelf_reflect   = float(sd.get("light_shelf_reflect",   0.80)),
            light_shelf_inside_depth_mm = float(sd.get("light_shelf_inside_depth_mm", 0.0)),
            light_shelf_outside_depth_mm = float(sd.get("light_shelf_outside_depth_mm", 0.0)),
            light_shelf_tilt_deg = float(sd.get("light_shelf_tilt_deg", 0.0)),
            light_shelf_height_mm = float(sd.get("light_shelf_height_mm", 0.0)),
            light_shelf_upper_window_ratio = float(sd.get("light_shelf_upper_window_ratio", 0.35)),
        )
        ...
        if wd_data and wd_data.get("monthly_lux"):
            ...
            if len(lux) == 12:
                ...
                weather = WeatherDataset(
                    ...
                    monthly_lux  = lux,
                    monthly_temp = temp,
                    latitude     = wd_data.get("latitude"),
                    longitude    = wd_data.get("longitude"),
                    data_period  = wd_data.get("data_period", ""),
                )
