"""
ui/experiment_sidebar.py — 参数化实验专用侧边栏  v4.4.1
================================================================
...
class ExperimentSidebar(QWidget):
    ...
    def _build_title(self):
        ...
        hint = QLabel("💡 可同时扫描固定挑檐、百叶和反光板；程序先找L1单构件可行域，"
                      "必要时再复算L2组合，并报告未达标瓶颈。")
    ...
    def _build_device_section(self):
        sec = CollapsibleSection("构件类型与干预层级", expanded=True)
        self._device_overhang = QCheckBox("固定挑檐")
        self._device_louver = QCheckBox("水平百叶")
        self._device_shelf = QCheckBox("内外反光板/导光板")
        self._device_overhang.setChecked(True)
        for widget in (self._device_overhang, self._device_louver, self._device_shelf):
            sec.add_widget(widget)
        self._enable_combinations = QCheckBox("L1无可行解时复算L2两构件组合")
        self._enable_combinations.setChecked(True)
        sec.add_widget(self._enable_combinations)
        self._combination_seeds = self._spin(1, 5, 2)
        sec.add_row("每类组合种子", self._combination_seeds)
        note = QLabel(
            "停止规则：L0基准→L1单构件→L2组合构件→L3主动补偿。"
            "一旦较低层级出现满足全部硬约束的方案，就不再用更复杂方案冒充最优。"
        )
    ...
    def _build_louver_section(self):
        sec = CollapsibleSection("百叶参数（角度 × 片宽 × 间距）", expanded=False)
        self._lv_angle_min = self._spin(0, 90, 0, 5)
        self._lv_angle_max = self._spin(0, 90, 60, 5)
        self._lv_angle_step = self._spin(1, 45, 15, 5)
        self._range_row(sec, "叶片角度(°)", self._lv_angle_min, self._lv_angle_max,
                        self._lv_angle_step, "°")
        self._lv_width_min = self._spin(20, 500, 80, 10)
        self._lv_width_max = self._spin(20, 500, 120, 10)
        self._lv_width_step = self._spin(1, 200, 40, 10)
        self._range_row(sec, "叶片宽(mm)", self._lv_width_min, self._lv_width_max,
                        self._lv_width_step, "mm")
        self._lv_spacing_min = self._spin(20, 1000, 100, 10)
        self._lv_spacing_max = self._spin(20, 1000, 150, 10)
        self._lv_spacing_step = self._spin(1, 500, 50, 10)
        self._range_row(sec, "叶片距(mm)", self._lv_spacing_min, self._lv_spacing_max,
                        self._lv_spacing_step, "mm")
        self._lv_position = QComboBox()
        self._lv_position.addItem("室外百叶", "exterior")
        self._lv_position.addItem("中空百叶", "between")
        self._lv_position.addItem("室内百叶", "interior")
        sec.add_row("安装位置", self._lv_position)
        self._lv_control = QComboBox()
        self._lv_control.addItem("固定角度", "fixed")
        self._lv_control.addItem("季节控制（5—9月展开）", "seasonal")
        sec.add_row("控制方式", self._lv_control)
        note = QLabel("静态采光图显示展开态；季节控制只在逐月热工/运行费中按月生效。"
                      "当前月均模型不虚构小时级自动控制结果。")
        note.setWordWrap(True)
        note.setStyleSheet("color:#9aa0b0;font-size:10px;background:transparent;")
        sec.add_widget(note)
    ...
    def _build_shelf_section(self):
        sec = CollapsibleSection("反光板参数（内伸 × 外挑 × 上窗比）", expanded=False)
        self._sh_in_min = self._spin(0, 2000, 0, 100)
        self._sh_in_max = self._spin(0, 2000, 300, 100)
        self._sh_in_step = self._spin(1, 1000, 300, 50)
        self._range_row(sec, "内伸(mm)", self._sh_in_min, self._sh_in_max,
                        self._sh_in_step, "mm")
        self._sh_out_min = self._spin(0, 3000, 300, 100)
        self._sh_out_max = self._spin(0, 3000, 900, 100)
        self._sh_out_step = self._spin(1, 1000, 300, 50)
        self._range_row(sec, "外挑(mm)", self._sh_out_min, self._sh_out_max,
                        self._sh_out_step, "mm")
        self._sh_tilt_min = self._spin(-30, 30, 0, 5)
        self._sh_tilt_max = self._spin(-30, 30, 0, 5)
        self._sh_tilt_step = self._spin(1, 30, 5, 5)
        self._range_row(sec, "板倾角(°)", self._sh_tilt_min, self._sh_tilt_max,
                        self._sh_tilt_step, "°")
        self._sh_ratio_min = self._spin(10, 80, 30, 5)
        self._sh_ratio_max = self._spin(10, 80, 50, 5)
        self._sh_ratio_step = self._spin(1, 40, 10, 5)
        self._range_row(sec, "上窗比(%)", self._sh_ratio_min, self._sh_ratio_max,
                        self._sh_ratio_step, "%")
        note = QLabel("板高由上窗比例确定。程序同时评价深部补光、近窗遮挡、均匀度和太阳得热。")
        note.setWordWrap(True)
        note.setStyleSheet("color:#9aa0b0;font-size:10px;background:transparent;")
        sec.add_widget(note)
        self._main.addWidget(sec)
    ...
    def _build_material_section(self):
        ...
        for cat, mats in MATERIAL_LIBRARY.items():
            ...
            for name, spec in mats.items():
                ...
                cb.setToolTip(
                    f"可见光反射率={spec['visible_reflectance']:.2f}；"
                    f"太阳反射率={spec['solar_reflectance']:.2f}；"
                    f"热发射率={spec['thermal_emissivity']:.2f}；"
                    f"镜面比例={spec['specular_fraction']:.2f}；"
                    f"k_diff={spec['k_diff']:.2f}{range_text}；"
                    f"{spec.get('price_basis', '')}；材料可见光反射率影响挑檐一次反射补光，"
                    "太阳反射率与发射率影响热环境筛选结果")
    ...
    def _build_project_section(self):
        ...
        note = QLabel(
            ...
            "移除原有遮阳，再扫描新的挑檐、百叶和反光板。原工程本身不会被修改。")
        note.setWordWrap(True)
        note.setStyleSheet("color:#9aa0b0;font-size:10px;background:transparent;")
        sec.add_widget(note)
    ...
    def _build_operation_section(self):
        sec = CollapsibleSection("运营时段与电费（第三目标）", expanded=True)
        note = QLabel(
            "第三目标按幼儿园实际使用时段计算：年照明电费＋年空调电费＋"
            "遮阳工程造价年化。固定多媒体、消毒等不受遮阳影响的用电不参与方案排名。"
        )
        note.setWordWrap(True)
        note.setStyleSheet("color:#9aa0b0;font-size:10px;background:transparent;")
        sec.add_widget(note)
        ...
        self._building_type.addItem("幼儿园", userData="幼儿园")
        self._building_type.setToolTip(
            "本版仅开放幼儿园；数据结构已预留办公、学校和商业建筑类型。"
        )
        sec.add_row("建筑类型", self._building_type)

        self._room_use = QComboBox()
        self._room_use.addItem("普通班级活动室")
        self._room_use.addItem("公共功能活动室")
        self._room_use.currentTextChanged.connect(self._apply_room_use_defaults)
        sec.add_row("空间用途", self._room_use)

        def numeric(value, minimum, maximum, step, suffix, decimals=1):
            spin = QDoubleSpinBox()
        ...
        self._teaching_days = numeric(200, 1, 365, 5, " 天/年", 0)
        self._lighting_hours = numeric(7.5, 0, 24, 0.5, " h/日", 1)
        self._hvac_hours = numeric(6.0, 0, 24, 0.5, " h/日", 1)
        self._lighting_power_density = numeric(9.0, 0, 30, 0.5, " W/㎡", 1)
        self._electricity_tariff = numeric(0.85, 0, 5, 0.05, " 元/kWh", 2)
        self._cooling_cop = numeric(3.2, 0.1, 10, 0.1, "", 1)
        self._heating_cop = numeric(3.0, 0.1, 10, 0.1, "", 1)
        self._service_life = numeric(20, 1, 100, 1, " 年", 0)
        for label, widget in (
            ("教学天数", self._teaching_days),
            ("照明时长", self._lighting_hours),
            ("空调占用时长", self._hvac_hours),
            ("照明功率密度", self._lighting_power_density),
            ("电价", self._electricity_tariff),
            ("制冷COP", self._cooling_cop),
            ("制热COP", self._heating_cop),
            ("遮阳使用年限", self._service_life),
        ):
            sec.add_row(label, widget)
        detail = QLabel(
            "默认普通活动室：200教学日、照明7.5h/日、有人空调6h/日、"
            "LED 9W/㎡。Cd用于估算自然光可替代的照明比例；热耗按18~26℃"
            "之外的占用度时计算。该月均准稳态口径用于方案筛选，论文定案仍建议"
            "用逐时能耗软件复核。"
        )
        detail.setWordWrap(True)
        detail.setStyleSheet("color:#9aa0b0;font-size:10px;background:transparent;")
        sec.add_widget(detail)
        self._main.addWidget(sec)

    def _apply_room_use_defaults(self, room_use: str) -> None:
        if room_use == "公共功能活动室":
            self._lighting_hours.setValue(2.5)
            self._hvac_hours.setValue(2.5)
        else:
            self._lighting_hours.setValue(7.5)
            self._hvac_hours.setValue(6.0)
    ...
    def _build_decision_section(self):
        sec = CollapsibleSection("论文预注册硬约束（可行域）", expanded=True)
        note = QLabel(
            "阈值必须在看结果前确定。0表示关闭该约束；默认值仅用于试跑，"
            "不自动宣称为规范限值。无可行解时程序报告最大归一化违反度和主导瓶颈。"
        )
        note.setWordWrap(True)
        note.setStyleSheet("color:#9aa0b0;font-size:10px;background:transparent;")
        sec.add_widget(note)
        def constraint_spin(value, maximum=10000.0, decimals=2):
            widget = QDoubleSpinBox()
            widget.setRange(0.0, maximum)
            widget.setDecimals(decimals)
            widget.setSingleStep(0.05 if decimals else 1.0)
            widget.setValue(value)
            widget.setFixedHeight(28)
            return widget
        self._c_cd = constraint_spin(0.80, 1.0)
        self._c_ra = constraint_spin(0.75, 1.0)
        self._c_u0 = constraint_spin(0.0, 1.0)
        self._c_thermal = constraint_spin(60.0, 1000.0, 1)
        self._c_cost = constraint_spin(0.0, 10000000.0, 0)
        sec.add_row("Cd ≥", self._c_cd)
        sec.add_row("Ra ≥", self._c_ra)
        sec.add_row("U0 ≥", self._c_u0)
        sec.add_row("热不舒适度 ≤", self._c_thermal)
        sec.add_row("年综合费用 ≤", self._c_cost)
        self._minimum_criterion = QComboBox()
        self._minimum_criterion.addItem("构件数量最少", "component_count")
        self._minimum_criterion.addItem("初投资最低", "construction_cost")
        self._minimum_criterion.addItem("年综合费用最低", "annual_total_cost")
        self._minimum_criterion.addItem("生命周期成本最低", "lifecycle_total_cost")
        sec.add_row("层级内最小口径", self._minimum_criterion)
        self._main.addWidget(sec)
    ...
    def _on_run(self):
        device_types = []
        if self._device_overhang.isChecked():
            device_types.append("horizontal_overhang")
        if self._device_louver.isChecked():
            device_types.append("louver")
        if self._device_shelf.isChecked():
            device_types.append("light_shelf")
        if not device_types:
            QMessageBox.warning(self, "提示", "请至少勾选一种构件类型。"); return
        ...
        for lower, upper, label in (
            (self._lv_angle_min, self._lv_angle_max, "百叶角度"),
            (self._lv_width_min, self._lv_width_max, "百叶片宽"),
            (self._lv_spacing_min, self._lv_spacing_max, "百叶间距"),
            (self._sh_in_min, self._sh_in_max, "反光板内伸"),
            (self._sh_out_min, self._sh_out_max, "反光板外挑"),
            (self._sh_tilt_min, self._sh_tilt_max, "反光板倾角"),
            (self._sh_ratio_min, self._sh_ratio_max, "反光板上窗比"),
        ):
            if upper.value() < lower.value():
                QMessageBox.warning(self, "提示", f"{label}上限不能小于下限。"); return
        ...
        n_geom = (
            len(tilt_degs) * len(positive_depths) * len(gap_mms)
            if "horizontal_overhang" in device_types else 0
        )
        louver_angles = self._range_list(
            self._lv_angle_min.value(), self._lv_angle_max.value(),
            self._lv_angle_step.value())
        louver_widths = self._range_list(
            self._lv_width_min.value(), self._lv_width_max.value(),
            self._lv_width_step.value())
        louver_spacings = self._range_list(
            self._lv_spacing_min.value(), self._lv_spacing_max.value(),
            self._lv_spacing_step.value())
        shelf_inside = self._range_list(
            self._sh_in_min.value(), self._sh_in_max.value(), self._sh_in_step.value())
        shelf_outside = self._range_list(
            self._sh_out_min.value(), self._sh_out_max.value(), self._sh_out_step.value())
        shelf_tilts = self._range_list(
            self._sh_tilt_min.value(), self._sh_tilt_max.value(), self._sh_tilt_step.value())
        shelf_ratios = [value / 100.0 for value in self._range_list(
            self._sh_ratio_min.value(), self._sh_ratio_max.value(), self._sh_ratio_step.value())]
        n_louver = (
            len(louver_angles) * len(louver_widths) * len(louver_spacings)
            if "louver" in device_types else 0
        )
        n_shelf = (
            len(shelf_inside) * len(shelf_outside) * len(shelf_tilts) * len(shelf_ratios)
        ...
        n_pts = (n_geom + n_louver + n_shelf) * len(materials) + 1 + no_shade_extra
        if n_pts > 200:
            r = QMessageBox.question(
                self, "组合较多",
                f"当前设置将产生约 {n_pts} 个L0/L1数据点（挑檐{n_geom}＋百叶"
                f"{n_louver}＋反光板{n_shelf}）×"
        ...
        params = {
            ...
            "louver_params": {
                "angles": louver_angles, "widths": louver_widths,
                "spacings": louver_spacings,
                "positions": [str(self._lv_position.currentData())],
                "controls": [str(self._lv_control.currentData())],
            },
            "light_shelf_params": {
                "inside_depths": shelf_inside, "outside_depths": shelf_outside,
                "tilts": shelf_tilts, "upper_ratios": shelf_ratios,
            },
            "materials": materials,
            "material_unit_costs": {
            ...
            "operation_profile": {
                "building_type": str(self._building_type.currentData()),
                "room_use": self._room_use.currentText(),
                "teaching_days_per_year": float(self._teaching_days.value()),
                "lighting_hours_per_day": float(self._lighting_hours.value()),
                "hvac_hours_per_day": float(self._hvac_hours.value()),
                "lighting_power_density_w_m2": float(
                    self._lighting_power_density.value()
                ),
                "electricity_tariff_yuan_kwh": float(
                    self._electricity_tariff.value()
                ),
                "cooling_cop": float(self._cooling_cop.value()),
                "heating_cop": float(self._heating_cop.value()),
                "construction_service_life_years": float(
                    self._service_life.value()
                ),
            },
            "cost_basis": (
                "年照明电费 + 年空调电费 + 遮阳工程造价/使用年限"
            ),
            "y": ykey, "maximize_y": _Y_META[ykey][0], "y_label": _Y_META[ykey][1],
            "u0_min": float(self._u0_min.value()),
            "constraints": {
                "daylight_score_min": float(self._c_cd.value()),
                "ra_min": float(self._c_ra.value()),
                "u0_min": float(self._c_u0.value()),
                "thermal_discomfort_max": float(self._c_thermal.value()),
                "annual_total_cost_max": float(self._c_cost.value()),
            },
            "minimum_criterion": str(self._minimum_criterion.currentData()),
            "enable_combinations": self._enable_combinations.isChecked(),
            "combination_seed_count": int(self._combination_seeds.value()),
        }
