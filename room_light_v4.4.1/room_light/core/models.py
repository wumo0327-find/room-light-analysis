"""
core/models.py — Legacy rectangular data models  v4.4.1
...
class ShadingDevice:
    ...
    水平挑檐互相独立、可同时生效）；v4.4.0 实现固定/季节百叶与内外组合反光板，
    并允许多个被动构件组合，用于论文的最小干预层级比较；
    ...
    # 关于斜面射线相交判定的推导；v4.2.2起热工SC使用倾斜剖面阴影式，仍以
    ...
    # v4.2.2: 遮阳板表面光热属性。旧工程缺少这些字段时使用保守默认值，
    ...
    # v4.4.0: 组合构件。空列表时由 type 推断，保持旧工程兼容；非空时可包含
    # horizontal_overhang / louver / light_shelf。target_window_ids 为空表示全部窗。
    components: List[str] = field(default_factory=list)
    target_window_ids: List[str] = field(default_factory=list)

    # 百叶（水平叶片；角度0°近水平，90°近竖直）
    ...
    louver_position: str = "exterior"  # exterior | between | interior
    louver_control_mode: str = "fixed" # fixed | seasonal（5—9月展开）

    # 反光板/导光板。旧 light_shelf_depth_mm 保留并视为外挑深度的兼容值。
    light_shelf_depth_mm: float = 0.0   # mm 旧字段/外挑深度别名
    light_shelf_reflect:  float = 0.80  # -  反射率
    light_shelf_inside_depth_mm: float = 0.0
    light_shelf_outside_depth_mm: float = 0.0
    light_shelf_tilt_deg: float = 0.0   # ° 正值向室内上扬
    light_shelf_height_mm: float = 0.0  # mm 相对地面；0按上窗比例反算
    light_shelf_upper_window_ratio: float = 0.35

    def enabled_components(self) -> List[str]:
        """Return active passive-device kinds, preserving old ``type`` files."""
        values = [str(value) for value in self.components if str(value)]
        if not values and self.type not in {"", "none"}:
            values = [self.type]
        return list(dict.fromkeys(values))

    def has_component(self, kind: str) -> bool:
        return kind in self.enabled_components()

    def applies_to(self, win_id) -> bool:
        targets = {str(value) for value in self.target_window_ids}
        return not targets or str(win_id) in targets

    def effective_shelf_outside_depth_mm(self) -> float:
        return max(
            0.0,
            float(self.light_shelf_outside_depth_mm),
            float(self.light_shelf_depth_mm),
        )

    def louver_deployed(self, month: Optional[int] = None) -> bool:
        if not self.has_component("louver"):
            return False
        if self.louver_control_mode != "seasonal" or month is None:
            return True
        return 5 <= int(month) <= 9

    def louver_direct_transmittance(self, profile_angle_deg: float) -> float:
        """Periodic-slat projected aperture ratio for screening calculations."""
        import math
        spacing = max(float(self.louver_spacing_mm), 1e-6)
        width = max(float(self.louver_width_mm), 0.0)
        alpha = math.radians(float(profile_angle_deg))
        theta = math.radians(float(self.louver_angle_deg))
        blockage = width * abs(math.sin(theta - alpha)) / spacing
        return max(0.0, min(1.0, 1.0 - blockage))
    ...
    def SC_reduction_factor(self) -> float:
        ...
        百叶/反光板的太阳位置相关修正由热工引擎逐月计算，此处仍返回1.0。
        """
        return 1.0
    ...
    def beam_shade_fraction(self, profile_angle_deg, window_height_mm: float,
        ...
        tilt_deg：θ=90°水平（默认）。v4.2.2起按太阳剖面内的板尖水平和竖向位移
        ...
        if (not self.has_component("horizontal_overhang")
...
MATERIAL_LIBRARY = {
    "混凝土类": {
        "深色/多孔混凝土":   {"reflect": 0.16, "k_diff": 0.20, "color": "#e11d48", "installed_cost_per_m2": 250.0, "cost_range_per_m2": (220.0, 280.0), "price_basis": "50~60mm预制板，含模具摊销、配筋、深色着色、养护及综合损耗"},
        "密实混凝土":        {"reflect": 0.32, "k_diff": 0.30, "color": "#7c3aed", "installed_cost_per_m2": 230.0, "cost_range_per_m2": (200.0, 260.0), "price_basis": "C30/C40、约50mm厚预制板，含配筋、模板、加工、运输及综合损耗"},
        "白色反射涂料混凝土": {"reflect": 0.65, "k_diff": 0.46, "color": "#f59e0b", "installed_cost_per_m2": 275.0, "cost_range_per_m2": (240.0, 310.0), "price_basis": "密实混凝土基层加底漆及白色反射面漆系统"},
    },
    "金属类": {
        ...
        "普通/氧化铝板":     {"reflect": 0.50, "k_diff": 0.38, "color": "#0284c7", "installed_cost_per_m2": 370.0, "cost_range_per_m2": (320.0, 420.0), "price_basis": "2.5~3.0mm铝单板，含普通粉末喷涂或自然氧化、加工及综合损耗"},
        "镜面高反射铝板":    {"reflect": 0.78, "k_diff": 0.55, "color": "#06b6d4", "installed_cost_per_m2": 550.0, "cost_range_per_m2": (480.0, 620.0), "price_basis": "高纯铝板，含机械抛光、阳极氧化或保护覆膜及较高运输损耗"},
    },
    "木材类": {
        ...
        "浅色木材(松木)":    {"reflect": 0.40, "k_diff": 0.34, "color": "#ea580c", "installed_cost_per_m2": 320.0, "cost_range_per_m2": (260.0, 380.0), "price_basis": "樟子松/南方松，含烘干定型、防腐防虫、切割组装及户外木蜡油"},
    },
    "涂料/其他类": {
        "基层遮阳板+深色涂层":       {"reflect": 0.15, "k_diff": 0.18, "color": "#15803d", "installed_cost_per_m2": 430.0, "cost_range_per_m2": (380.0, 480.0), "price_basis": "以2.5mm铝板为统一基层，含深色氟碳/外墙涂层系统、加工及综合损耗"},
...
# v4.2.2 光热属性补全。保留 ``reflect`` 旧字段作为论文既有数据口径，
