"""
core/experiments.py — 参数化实验批量运行 + 可追溯帕累托分析  v4.4.1
================================================================================
...
v2.14.0 图示语义：
  ...
  · 金色星标优先表示预注册硬约束下的最低层级决策推荐；旧结果缺少该字段时
    才退回等权归一化理想点距离最小方案。
...
def annotate_global_selection(
    ...
    if not candidates.empty and {light_col, thermal_col, "cost"}.issubset(candidates.columns):
        distances = np.sqrt((
            (1.0 - _benefit(candidates[light_col], True)) ** 2
            + (1.0 - _benefit(candidates[thermal_col], maximize_thermal)) ** 2
            + (1.0 - _benefit(candidates["cost"], False)) ** 2
        ) / 3.0)
...
def global_selection_summary(
    ...
    if not rec.empty:
        ...
        recommendation = (
            f"决策推荐（金星）：{rec_name}。新结果按预注册硬约束与最小干预"
            "停止规则产生；若文件缺少决策字段，才退回三目标等权理想点距离。"
        )
    else:
