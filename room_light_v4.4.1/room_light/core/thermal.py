"""
core/thermal.py — Indoor Thermal Environment Engine  v2.12.0
============================================================
...
class ThermalResult:
    ...
    # 连续热舒适指标：同时反映偏离舒适区的月数与温差幅度。
    overheat_degree_months: float = 0.0
    underheat_degree_months: float = 0.0
...
def compute_thermal(
    ...
    k_diff  = s.effective_shaded_solar_residual()
    win_sc_monthly = {}   # win.id -> [12] 逐月有效 SC
    for win in room.windows:
        profs = solar.monthly_noon_profile_angles(lat_deg, win.wall, orient)
        depth_mm, gap_mm, tilt_deg = s.get_overhang_for(win.id)   # v2.10逐窗覆盖, v2.12.0新增tilt
        values = []
        for m in range(12):
            profile = profs[m]
            transmission = 1.0
            if s.applies_to(win.id) and s.has_component("horizontal_overhang"):
                transmission *= 1.0 - s.beam_shade_fraction(
                    profile, win.height, depth_mm=depth_mm, gap_mm=gap_mm,
                    tilt_deg=tilt_deg,
                ) * (1.0 - k_diff)
            if s.applies_to(win.id) and profile is not None and s.louver_deployed(m + 1):
                aperture = s.louver_direct_transmittance(profile)
                effectiveness = {"exterior": 1.0, "between": 0.72, "interior": 0.38}.get(
                    s.louver_position, 1.0
                )
                transmission *= 1.0 - (1.0 - aperture) * effectiveness * (
                    1.0 - 0.12 * s.louver_reflect
                )
            if s.applies_to(win.id) and s.has_component("light_shelf") and profile is not None:
                ratio = max(0.05, min(0.90, s.light_shelf_upper_window_ratio))
                lower_height = win.height * (1.0 - ratio)
                block = min(1.0, max(0.0,
                    s.effective_shelf_outside_depth_mm() * (
                        math.tan(math.radians(profile))
                        + math.tan(math.radians(s.light_shelf_tilt_deg))
                    ) / max(lower_height, 1e-6)
                )) * (1.0 - ratio)
                transmission *= 1.0 - block * (1.0 - 0.10 * s.light_shelf_reflect)
            values.append(t.SC_glass * max(0.0, min(1.0, transmission)))
        win_sc_monthly[win.id] = values
    # 逐月面积加权有效 SC（用于结果报告）
    ...
    res.overheat_degree_months = float(np.sum(
        np.maximum(T_smoothed - T_COMFORT_HIGH, 0.0)
    ))
    res.underheat_degree_months = float(np.sum(
        np.maximum(T_COMFORT_LOW - T_smoothed, 0.0)
    ))
    res.thermal_discomfort = (
        res.overheat_degree_months + res.underheat_degree_months
    )
