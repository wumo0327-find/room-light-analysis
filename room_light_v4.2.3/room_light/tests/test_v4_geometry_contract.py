...
class V4GeometryContractTests(unittest.TestCase):
    ...
    def test_direct_bplan_source_survives_complete_project_archive(self):
        source = self._rectangle()
        with tempfile.TemporaryDirectory() as directory:
            bplan_path = Path(directory) / "source.bplan.json"
            project_path = Path(directory) / "archive.rlproj"
            source.save(bplan_path)
            loaded_source = type(source).load(bplan_path)
            building, space = self._convert(loaded_source)
            source_fingerprint = audit_building_geometry(building).fingerprint
            save_building_project(
                str(project_path), building, active_space_id=space.id
            )
            restored, _weather, _active_id, error = load_building_project(
                str(project_path)
            )
        self.assertEqual(error, "")
        restored_report = audit_building_geometry(restored)
        self.assertTrue(restored_report.ok, restored_report.issues)
        self.assertEqual(restored_report.fingerprint, source_fingerprint)
