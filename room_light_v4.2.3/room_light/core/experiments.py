"""
core/experiments.py — 参数化实验批量运行 + 可追溯帕累托分析  v4.2.3
================================================================================
...
倾角、板长和间隙放入同一个候选池，按「连续采光达标度Cd最大、热不舒适度最小、成本最小」
...
v2.14.0 图示语义：
  ...
  · 3D轴保留Cd/热指标/成本原始值，并与三张2D图共用坐标范围；三个投影面
    在各目标较差边界相交于空间参考点O（O不要求数值为0）；热轴和成本轴
    反向显示，使低不舒适度、低成本与高Cd共同朝向最外侧。
...
def global_selection_summary(
    ...
    rule = (
        ...
        f"{u0_rule}若另一方案同时满足 {light_label}不低、{thermal_rule}、成本不高，"
        ...
        for material in material_order:
            ...
            if dominated_same_geometry == len(material_rows) and len(material_rows):
                ...
                reasons.append(
                    ...
                    f"均被{names}支配（{light_label}不低，{thermal_rule}且成本不高，至少一项更优）"
                )
            else:
                reasons.append(
    ...
    if not rec.empty:
        ...
        recommendation = (
            f"均衡推荐（金星）：{rec_name}。先把{light_label}、{thermal_label}和成本分别转为"
...
def build_pareto3d_figure(
    ...
    采用原始指标值构建透视3D图，返回(fig, ax)：
      X=连续采光达标度Cd（旧CSV回退Ra）、Y=成本、Z=所选热指标。
    ...
    # O位于三个目标的较差参考角：低采光优度、高热不舒适度（若越小越好）、高成本。
    ...
    ax.set_ylabel("工程造价估算（元） ↓", color=_C_TEXT, fontsize=9, labelpad=8)
