...
class _NoScrollSpinBox(QDoubleSpinBox):
    """
    修复：滚动侧边栏时不小心调节数值的问题。
    只有在控件已获得键盘焦点（用户主动点击选中）时，才响应鼠标滚轮。
    未聚焦时滚轮事件传递给父级 ScrollArea，正常上下滚动页面。
    """
    def wheelEvent(self, event):
        if self.hasFocus():
            super().wheelEvent(event)
        else:
            event.ignore()   # 交给父级 ScrollArea 处理滚动
...
def _lbl(text: str, bold=False, color="#e8edf5", size=13) -> QLabel:
    l = QLabel(text)
    f = QFont()
    f.setBold(bold)
    f.setPointSize(size)
