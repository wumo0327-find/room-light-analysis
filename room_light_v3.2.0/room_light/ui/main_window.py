...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("建筑室内采光分析工具  v3.2.0  |  益阳默认气象")
    ...
    def _open_complex_space_editor(self):
        ...
        self._selected_space_ids = [
            ...
            if self.building.get_space(space_id) is not None
        ]
        if (
            self._active_space_id
            and self._active_space_id not in self._selected_space_ids
        ):
            self._selected_space_ids.append(self._active_space_id)
        self._btn_run.setText("▶  分析选中房间")
        self.complex_canvas.set_model(
            self.building, self._active_space_id, self._selected_space_ids)
        self.complex_sidebar.set_model(
    ...
    def _update_title(self):
        ...
        suf = f"  —  {fn}" if fn else ""
        self.setWindowTitle(f"建筑室内采光分析工具  v3.2.0{suf}")
    ...
    def _run_experiment(self, params: dict):
        ...
        if not spaces:
            ...
            return
        from core.complex_experiments import exterior_windows
        missing_windows = [
            space.name for space in spaces if not exterior_windows(space)
        ]
        if missing_windows:
            QMessageBox.information(
                self,
                "提示",
                "以下选中房间没有外窗，无法运行遮阳参数化实验：\n"
        ...
        def _run_exp(progress_cb):
            ...
            for source_space in space_snaps:
                optimal_space = build_solution_space(source_space, row)
