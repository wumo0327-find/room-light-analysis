"""
core/plan_export.py — 房间平面图绘制与测点叠加导出  v2.12.0（v2.8.0 新增，v2.9.1 天正风格标注+遮阳可视化，v2.10.0 支持逐窗/逐位置遮阳覆盖+两端外墙翼板，v2.10.2 修正端墙柱宽为固定值非整段端墙宽度，v2.12.0 平面图按倾斜角θ换算水平投影深度）
======================================================================
...
def dim_arrow(ax, x1, y1, x2, y2, label, direction="h", fontsize=10):
    ...
    等场景），v2.9.1 起内部实现已从箭头式改为天正风格，不再画箭头——程序里所有
...
def _fin_slots(room: RoomModel, wall: str) -> List[Tuple[float, float, float]]:
    """
    v2.10: 该墙全部垂直翼板位置 [(u0, u1, depth_mm), ...]，含两端外墙位置
    （最左窗左侧、最右窗右侧）与窗间墙位置，深度已按逐位置覆盖
    （ShadingDevice.get_fin_depth_for）解析，深度≤0 的位置不返回（不生成翼板）。
    与 core/daylight.py 的 _fin_slots_full_m 使用同一套槽位定义，确保平面图
    画出的柱子形状与实际参与遮挡计算的形状一致。

    v2.10.2 修正: 两端外墙位置的柱子宽度是固定值 room.shading.
    fin_column_width_mm（现场实测，与窗间墙宽度数值巧合相同但概念不同），
    贴着窗户边缘、只占这么宽，不是整段端墙剩余宽度（v2.10.0/v2.10.1 曾
    错误地画成整段端墙宽度）。窗间墙位置的柱子仍按真实窗间墙宽度（该宽度
    本身就等于现场柱宽）。
    """
    sh = room.shading
    ...
    if not wins:
        return []
    axis_len = _wall_axis_len(room, wall)
    col_w = max(0.0, sh.fin_column_width_mm)
    slots = []
    d0 = sh.get_fin_depth_for(wins[0].id, "L")
    if d0 > 0.0 and wins[0].x > 1e-6:
        w0 = min(col_w, wins[0].x)
        if w0 > 1e-6:
            slots.append((max(0.0, wins[0].x - w0), wins[0].x, d0))
    for i in range(len(wins) - 1):
        u0, u1 = wins[i].right, wins[i + 1].x
        if u1 <= u0 + 1e-6:
            continue
        ...
        d = max(d_r, d_l)
        if d > 0.0:
            slots.append((u0, u1, d))
    d_last = sh.get_fin_depth_for(wins[-1].id, "R")
    if d_last > 0.0 and wins[-1].right < axis_len - 1e-6:
        w_last = min(col_w, axis_len - wins[-1].right)
        if w_last > 1e-6:
            slots.append((wins[-1].right, wins[-1].right + w_last, d_last))
    return slots
...
def draw_dim_chain(ax, room: RoomModel, wall: str,
    ...
    # v2.10.2: 若两端外墙的"端墙"区段里有柱子（宽度固定为 fin_column_width_mm，
    # 贴窗边缘，通常小于整段端墙区段），把该区段进一步拆成"墙"+"柱"两段，
    # 与用户现场CAD图纸的标注粒度一致（如 1310墙+540柱，而不是笼统的1850端墙）。
    fin_slots_ends = {(u0, u1) for (u0, u1, _d) in _fin_slots(room, wall)}
    expanded = []
    for (u0, u1, kind, win) in segs:
        if kind == "wall":
            match = next((s for s in fin_slots_ends
                         if abs(s[0] - u0) < 1e-6 or abs(s[1] - u1) < 1e-6), None)
            if match and (match[1] - match[0]) < (u1 - u0) - 1e-6:
                if abs(match[1] - u1) < 1e-6:      # 柱贴在该段右端（西端墙场景）
                    expanded.append((u0, match[0], "wall", None))
                    expanded.append((match[0], match[1], "col", None))
                else:                                # 柱贴在该段左端（东端墙场景）
                    expanded.append((match[0], match[1], "col", None))
                    expanded.append((match[1], u1, "wall", None))
                continue
        expanded.append((u0, u1, kind, win))
    segs = expanded
    ...
    for i, (u0, u1, kind, win) in enumerate(segs):
        ...
        if kind == "window":
            ...
            label = f"柱{length:.0f}"   # v2.10.2: 端墙柱子（贴窗边缘，非整段端墙）
        else:
...
def draw_shading_plan(ax, room: RoomModel, fontsize=8, label_beyond_mm=0.0):
    """
    水平挑檐：按每扇窗的窗宽 × 出挑深度画外凸矩形（虚线边框+半透明填充），深度
             支持逐窗覆盖（ShadingDevice.get_overhang_for）。
    垂直翼板：按翼板所在区间的真实宽度 × 出挑深度画外凸矩形（宽度=真实窗间墙/
             端墙宽度，不是示意值）。v2.10 起两端外墙位置（最左窗左侧、最右窗
             右侧）与窗间墙位置一样会画出翼板（此前仅窗间墙），深度支持逐位置
             覆盖（ShadingDevice.get_fin_depth_for，见 _fin_slots）。
    两者可同时存在，独立绘制、独立标注出挑深度，返回 (是否画了挑檐, 是否画了翼板)。
    ...
    if has_overhang:
        ...
        for win in wins:
            L, _gap, tilt = sh.get_overhang_for(win.id)   # v2.10逐窗覆盖, v2.12.0新增tilt
            # 平面图画的是俯视投影，倾斜挑檐的水平投影深度=L·sinθ（θ=90°水平时=L）
            D = L * _math.sin(_math.radians(tilt))
            if D <= 0.0:
                continue
            ...
            if win is mid_win:
                label = (f"挑檐出挑{D:.0f}" if abs(tilt - 90.0) < 1e-6
                        else f"挑檐L={L:.0f} θ={tilt:.0f}°(投影{D:.0f})")
                _label_shading_depth(ax, room, win.wall, win.x, win.right,
                                     max(D, label_beyond_mm) + step,
                                     label, C_OVERHANG, fontsize)

    fin_drawn = False
    if has_fin:
        for wall in ("south", "north", "east", "west"):
            slots = _fin_slots(room, wall)   # v2.10: 含两端外墙位置，逐位置覆盖深度
            mid_slot = slots[len(slots) // 2] if slots else None
            for (u0, u1, Df) in slots:
                fin_drawn = True
...
def draw_shading_elevation(ax, room: RoomModel, wall: str, fontsize=8):
    """
    在立面图上画该墙的遮阳构件示意（v2.9.1 新增，对应平面图的外凸矩形，
    改画正视投影）：
      · 水平挑檐：窗顶上方的横向薄板，宽度=窗宽，与平面一致，深度支持逐窗覆盖
        （出挑深度不影响立面厚度画法，立面厚度恒为示意值，仅采光/热工计算用
        实际深度，见下方注意事项）；
      · 垂直翼板：窗间墙/端墙位置的竖向薄板，宽度=真实窗间墙/端墙宽度、高度=
        窗台~窗顶（端墙位置取该窗自身窗台/窗顶；窗间墙位置取相邻两窗窗台/窗顶
        的外包范围，通常相邻窗高度相同）。v2.10 起两端外墙位置也画出（此前仅
        窗间墙），深度支持逐位置覆盖。
    ...
    overhang_drawn = False
    if has_overhang:
        for win in wins:
            L, _gap, _tilt = sh.get_overhang_for(win.id)
            D = L   # 立面厚度仍为示意值（未做倾斜面精确投影），沿用板长即可
            if D <= 0.0:
                continue
            overhang_drawn = True
            ax.add_patch(patches.Rectangle(
    ...
    fin_drawn = False
    if has_fin:
        def _draw_fin_rect(u0, u1, sill, head):
            nonlocal fin_drawn
            if u1 <= u0:
                return
            fin_drawn = True
        ...
        col_w = max(0.0, sh.fin_column_width_mm)   # v2.10.2: 端墙柱宽为固定值，非整段端墙宽度
        d0 = sh.get_fin_depth_for(wins[0].id, "L")
        if d0 > 0.0 and wins[0].x > 1e-6:
            w0 = min(col_w, wins[0].x)
            if w0 > 1e-6:
                _draw_fin_rect(wins[0].x - w0, wins[0].x, wins[0].sill, wins[0].head)
        for i in range(len(wins) - 1):
            d_r = sh.get_fin_depth_for(wins[i].id, "R")
            d_l = sh.get_fin_depth_for(wins[i + 1].id, "L")
            if max(d_r, d_l) > 0.0:
                ...
                _draw_fin_rect(wins[i].right, wins[i + 1].x, sill, head)
        d_last = sh.get_fin_depth_for(wins[-1].id, "R")
        if d_last > 0.0 and wins[-1].right < axis_len - 1e-6:
            w_last = min(col_w, axis_len - wins[-1].right)
            if w_last > 1e-6:
                _draw_fin_rect(wins[-1].right, wins[-1].right + w_last,
                              wins[-1].sill, wins[-1].head)

    return overhang_drawn, fin_drawn
...
def shading_dimension_lines(room: RoomModel) -> List[str]:
    ...
    if getattr(sh, "vertical_fin_enabled", False) and sh.vertical_fin_depth_mm > 0:
        lines.append(f"垂直翼板/装饰柱: 标注出挑{sh.vertical_fin_depth_mm:.0f}mm"
                     f"（计算时按含墙厚扣减，有效出挑{max(0.0, sh.vertical_fin_depth_mm - room.thermal.wall_thickness_mm):.0f}mm）  "
                     f"窗间墙位置柱宽=真实窗间墙宽度；两端外墙位置柱宽="
                     f"{sh.fin_column_width_mm:.0f}mm（贴窗边缘，非整段端墙宽度）")
    if not lines:
        lines.append("无遮阳构件")
    return lines
