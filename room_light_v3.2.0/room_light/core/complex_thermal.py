"""
core/complex_thermal.py — Arbitrary-space single-zone thermal engine  v3.2.0
...
def compute_complex_thermal(
    ...
    result.method = "单区集总热平衡 + 任意多边形围护结构 v3.2.0"
    ...
    for wall in exterior_walls:
        gross_area = wall.length_mm * space.height_mm / 1_000_000.0
    ...
    exterior_perimeter_m = sum(
        wall.length_mm / 1000.0 for wall in exterior_walls
    )
    exterior_corner_count = len(exterior_walls)
    bridge_length = (
        exterior_corner_count * height_m
    ...
    ua_roof = thermal.U_roof * floor_area
    ua_floor = thermal.U_floor * floor_area
