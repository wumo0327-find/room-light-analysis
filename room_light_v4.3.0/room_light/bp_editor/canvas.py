...
class DraftCanvas(QWidget):
    ...
    def _paint_elevation(self, painter: QPainter) -> None:
        ...
        # Individual projected wall rectangles overlap at corners, T-joints and
        # collinear splits.  Drawing every rectangle border exposes construction
        # seams that are not visible façade edges.  Outline only the union of all
        # projected wall faces; windows and railings keep their own necessary lines.
        if visible_walls:
            silhouette = QPainterPath()
            for wall in visible_walls:
                wall_path = QPainterPath()
                wall_path.addRect(self._elevation_wall_rect(wall))
                silhouette = (
                    wall_path
                    if silhouette.isEmpty()
                    else silhouette.united(wall_path)
                )
            painter.setBrush(Qt.BrushStyle.NoBrush)
            painter.setPen(QPen(WALL_EDGE, 1.2))
            painter.drawPath(silhouette.simplified())
    ...
    def _paint_elevation_wall(self, painter: QPainter, wall: Wall) -> None:
        rect = self._elevation_wall_rect(wall)
        # No per-wall outline: it would reveal hidden overlaps and model split
        # seams.  _paint_elevation draws the final visible silhouette once.
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(WALL_FILL)
        painter.drawRect(rect)

        painter.setBrush(WINDOW_COLOR)
        painter.setPen(QPen(WINDOW_EDGE, 1.1))
        for window in self.document.windows:
            ...
            if window_right - window_left <= 1.0:
                continue
    ...
    def _elevation_wall_rect(self, wall: Wall) -> QRectF:
