...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("建筑室内采光分析工具  v2.5.0  |  益阳默认气象")
    ...
    def _build_toolbar(self) -> QWidget:
        ...
        for b in (self._btn_arch, self._btn_anal, self._btn_thml):
    ...
    def _view_buttons(self): return (self._btn_arch, self._btn_anal, self._btn_thml)

    def _switch_view(self, idx: int):
        self._stack.setCurrentIndex(idx)
        labels = ["建筑视图", "采光分析", "热环境"]
    ...
    def _update_title(self):
        ...
        suf = f"  —  {fn}" if fn else ""
        self.setWindowTitle(f"建筑室内采光分析工具  v2.5.0{suf}")
