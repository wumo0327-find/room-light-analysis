"""
core/models.py — Data models  v2.10.2
...
class ShadingDevice:
    ...
    # 缺失该键的窗户使用上面的全局 overhang_depth_mm/overhang_height_mm。
    ...
    def get_overhang_for(self, win_id) -> tuple:
        """
        v2.10: 返回给定窗户实际生效的 (出挑深度mm, 安装间隙mm)。
        若该窗在 overhang_overrides 中有覆盖项，优先使用覆盖值（可只覆盖其中
        一项，另一项仍取全局默认）；否则使用全局 overhang_depth_mm/height_mm。
        """
        ...
        return (self.overhang_depth_mm, self.overhang_height_mm)
    ...
    def beam_shade_fraction(self, profile_angle_deg, window_height_mm: float,
                            ...
                            gap_mm: Optional[float] = None) -> float:
        """
        ...
        """
        import math
        D = self.overhang_depth_mm  if depth_mm is None else depth_mm
