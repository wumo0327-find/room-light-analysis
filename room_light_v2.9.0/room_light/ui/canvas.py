...
class RoomCanvas(QWidget):
    ...
    # ── Floor-plan view ───────────────────────────────────────────────────────
    # v2.9.0: 绘制逻辑已抽取到 core/plan_export.py（GUI 画布与命令行静态导出共用），
    # 本方法只是薄封装，行为与旧版逐像素一致。
    ...
    def _draw_elevation(self, wall: str) -> None:
        ...
        # 遮阳构件（v2.9.0 新增：立面上同步显示水平挑檐/垂直翼板示意）
    ...
    # ── Helpers ───────────────────────────────────────────────────────────────
    # v2.9.0: 委托给 core/plan_export.py 的共享实现（elevation 视图仍复用这两个）。
